...
const currentPart = computed(() =>
  courseIndex.curriculum.parts.find((part) => currentPath.value.endsWith(`/parts/${part.id}`)),
);
...
const currentChapter = computed(() =>
  allChapters.value.find((chapter) => currentPath.value.endsWith(`/outline/${chapter.id}`)),
);
...
const defaultFocusAreas = ["核心概念与问题模型", "表示、算法与复杂度", "习题、Lab 与边界验证"];
const currentObjectives = computed(() =>
  currentChapter.value?.learningObjectives?.length
...
const groups = computed(() =>
  mode.value === "part" && currentPart.value
    ? [
        {
          id: currentPart.value.id,
          label: `Part ${currentPart.value.numeral} · ${currentPart.value.title}`,
          url: currentPart.value.url,
          chapters: currentPart.value.chapters,
        },
      ]
    : [
        {
          id: "foundations",
          label: "基础部分",
          url: courseIndex.curriculum.url,
          chapters: courseIndex.curriculum.foundations,
        },
  ...
  <main class="course-curriculum-index">
    <header class="course-curriculum-hero">
      ...
      <template v-if="mode === 'chapter' && currentChapter">
        ...
        <span class="course-status-badge" :class="`is-${currentChapterStatus.key}`">{{
          currentChapterStatus.label
        }}</span>
      </template>
      ...
      <section class="course-curriculum-resources">
        ...
        <div v-if="currentChapter.lessons.length || currentChapter.labs.length" class="course-curriculum-resource-list">
          <a v-for="lesson in currentChapter.lessons" :key="lesson.url" :href="withBase(lesson.url)">
            <BookOpen aria-hidden="true" :size="18" /><span
              ><strong>{{ lesson.title }}</strong
              ><small>{{ lesson.order === 0 ? "章节导读" : "理论文章" }}</small></span
            ><ArrowRight aria-hidden="true" :size="16" />
          </a>
          <a v-for="lab in currentChapter.labs" :key="lab.url" :href="withBase(lab.url)" target="_self">
            <FlaskConical aria-hidden="true" :size="18" /><span
              ><strong>{{ lab.title }}</strong
              ><small>相关 Lab</small></span
            ><ArrowRight aria-hidden="true" :size="16" />
          </a>
        </div>
