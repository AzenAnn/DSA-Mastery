...
readonly LOCAL_SETUP="${SCRIPT_DIR}/../dist/setup.js"
...
valid_repository() {
  [[ -f "$1/package.json" && -f "$1/pnpm-lock.yaml" && -d "$1/labs" && -f "$1/packages/lab-cli/dist/cli.js" && -f "$1/packages/bootstrap/dist/setup.js" ]]
}
...
prepare_external_repository() {
  ...
  if ! valid_repository "$repo_dir"; then
    echo "clone 完成但未找到共享 setup.js：$repo_dir" >&2
    exit 13
  fi
}
...
if [[ ! -f "$setup_path" ]]; then
  setup_path="$repo_dir/packages/bootstrap/dist/setup.js"
fi

if [[ ! -f "$setup_path" ]]; then
  echo "找不到 DSA Mastery setup.js：$setup_path" >&2
  exit 13
fi
