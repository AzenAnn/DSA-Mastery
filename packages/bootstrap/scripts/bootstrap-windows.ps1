...
$LocalSetup = Join-Path $ScriptDir "..\dist\setup.js"
...
function Test-ValidRepository {
    ...
    return (Test-Path (Join-Path $Path "package.json") -PathType Leaf) -and
        ...
        (Test-Path (Join-Path $Path "packages\bootstrap\dist\setup.js") -PathType Leaf)
}
...
$SetupPath = if (Test-Path $LocalSetup -PathType Leaf) { $LocalSetup } else { Join-Path $RepoDir "packages\bootstrap\dist\setup.js" }
if (-not (Test-Path $SetupPath -PathType Leaf)) { throw "Cannot find DSA Mastery setup.js: $SetupPath" }
