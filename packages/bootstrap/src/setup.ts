...
import type { InputStream, OutputStream, ProgressUI, StageStatus, Stage } from "./ui.ts";
...
import { evaluateProfile, inspectHost, PNPM_VERSION } from "./checks.ts";
...
function setupError(code: string, message: string, details?: unknown): SetupError {
...
function resultOutput(result: Partial<CaptureResult> | undefined): string {
  return `${result?.stdout ?? ""}\n${result?.stderr ?? ""}`.trim();
}

function resultFailed(result: Partial<CaptureResult> | undefined): boolean {
  return (
    Boolean(result?.spawnError) || result?.code !== 0 || Boolean(result?.timedOut) || Boolean(result?.outputExceeded)
  );
}

function firstOutputLine(result: Partial<CaptureResult> | undefined): string | undefined {
  return resultOutput(result).split(/\r?\n/).find(Boolean)?.trim();
}

function prependPath(currentPath: string | undefined, additions: string[], delimiter: string): string {
...
function hasTool(host: Partial<HostReport> | undefined, name: string): boolean {
...
function wingetInstall(id: string, extra: string[] = []): { command: string; args: string[] } {
  return {
    command: "winget",
    args: [
      "install",
...
async function detectPackageManager(context: SetupContext): Promise<PackageManager | undefined> {
...
async function refreshPlatformEnvironment(context: SetupContext): Promise<NodeJS.ProcessEnv> {
...
async function resolveExecutable(context: SetupContext, command: string): Promise<string> {
...
async function ensureHomebrew(context: SetupContext): Promise<PackageManager> {
...
async function inspectContextHost(context: SetupContext): Promise<HostReport> {
...
async function ensureToolchain(context: SetupContext): Promise<ProfileEvaluation> {
...
async function ensureRepository(context: SetupContext): Promise<RepositoryState> {
...
async function askRepositoryUpdate(
  options: SetupOptions,
...
async function runSmoke(context: SetupContext) {
...
async function installIde(context: SetupContext): Promise<Record<string, unknown>> {
...
function serializeHost(host: HostReport | undefined) {
...
function summarizeReport(report: SetupReport): string {
  const lines = [
    "",
...
async function writeFailureLog(context: SetupContext, report: SetupReport): Promise<string | undefined> {
...
async function runCheckOnly(context: SetupContext): Promise<Record<string, unknown>> {
  const evaluation = evaluateProfile(context.profile, context.host!.tools);
  context.evaluation = evaluation;
...
function normalizeError(rawError: unknown): SetupError {
