...
interface LabConfig {
  ...
  labId: string;
  title: string;
  oracle: (inputText: string) => string;
  source?: string;
}
...
function containerArea(inputText: string) {
...
function jumpGame(inputText: string) {
  const nums = parseSizedArray(inputText);

  if (nums.length <= 500) {
    const reachable = Array.from<boolean>({ length: nums.length }).fill(false);
    reachable[0] = true;
...
function jumpGameTwo(inputText: string) {
  const nums = parseSizedArray(inputText);
  let jumps = 0;
  let boundary = 0;
  let farthest = 0;
...
function candy(inputText: string) {
...
function locateLab(sequence: number) {
...
function checkLab(config: LabConfig) {
  const directory = locateLab(config.sequence);
  const readme = read(path.join(directory, "README.md"));
  const manifest = JSON.parse(read(path.join(directory, "lab.json"))) as {
    type: string;
    toolchain?: { standard?: string };
    judge?: { compare?: { mode?: string } };
  };
