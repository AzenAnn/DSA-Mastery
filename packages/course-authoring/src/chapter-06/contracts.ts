...
export type Operation = [string, number, number];
...
const baseFor = (id: number): number => ([4, 5, 6, 7, 20, 21, 22].includes(id) ? 1 : 0);
export function maxN(id: number): number {
  return { 5: 1000, 6: 1000, 9: 500, 11: 100, 12: 200000, 13: 1000, 14: 200, 19: 50 }[id] ?? 100000;
}
export const isDirected = (id: number): boolean => [5, 8, 13, 17, 21, 22, 23].includes(id);
const line = (values: unknown[]): string => `${values.join(" ")}\n`;
export const edgeKey = (u: number, v: number): string => `${u},${v}`;
...
export function labelsDsu(n: number, edges: Edge[]): number[] {
...
function reachable(n: number, edges: Edge[], start = 0): boolean[] {
  const graph = adjacency(n, edges, true),
    seen = Array.from<boolean>({ length: n }).fill(false),
    queue = [start];
  seen[start] = true;
...
export function labelsTarjan(n: number, edges: Edge[]): number[] {
  const graph = adjacency(n, edges, true);
  const index = Array.from<number>({ length: n }).fill(0),
    low = Array.from<number>({ length: n }).fill(0),
    active = Array.from<boolean>({ length: n }).fill(false),
    raw = Array.from<number>({ length: n }).fill(0);
...
export function closure(n: number, edges: Edge[], directed = true): boolean[][] {
...
export function labelsClosure(a: boolean[][]): number[] {
...
export function serialize(id: number, data: GraphData): string {
  const { n, edges = [] } = data,
    base = baseFor(id);
  ...
  if (id === 6) {
    return (
      line([n, edges.length, data.queries!.length]) +
      edgeLines +
      data.queries!.map(([u, v]) => line([u + 1, v + 1])).join("")
    );
  }
  if (id === 9) {
    ...
    return (
      ...
      graph
        .map((row) => {
          ...
          return line(Array.from({ length: n }, (_, v) => Number(set.has(v))));
        })
        .join("")
    );
  }
  ...
  if (id === 13) {
    return `${n}\n${adjacency(n, edges, true)
      .map((row) => line([row.length, ...row]))
      .join("")}`;
  }
  if (id === 14) {
...
export function parseInput(id: number, text: string): GraphData {
  ...
  let type: string | undefined, format: string | undefined;
  if ([9, 10].includes(id)) {
    type = word();
    assert(["U", "D"].includes(type));
  }
  if (id === 9) {
    format = word();
    assert(["E", "M", "L"].includes(format));
  }
  const n = integer(id === 4 ? 3 : [11, 13].includes(id) ? 2 : 1, maxN(id)),
    base = baseFor(id);
  const vertex = () => integer(base, n - 1 + base) - base;
  const data: GraphData = { n, edges: [], type, format };
  const readEdges = (m: number) => {
    for (let i = 0; i < m; ++i) data.edges.push([vertex(), vertex()]);
  };
  if (id === 4) {
    readEdges(n - 1);
  } else if (id === 10) {
    ...
    for (let i = 0; i < q; ++i) {
      ...
      assert(["ADD", "DEL", "HAS"].includes(operation));
      const u = vertex(),
        v = vertex();
      assert.notEqual(u, v);
      operations.push([operation, u, v]);
    }
  } else if (id === 13 || (id === 9 && format === "L")) {
    for (let u = 0; u < n; ++u) {
      const k = integer(0, id === 13 ? Math.min(n, 1000) : n - 1),
        neighbors = new Set<number>();
      for (let i = 0; i < k; ++i) {
        const v = vertex();
        assert(!neighbors.has(v), "duplicate neighbor");
        neighbors.add(v);
        data.edges.push([u, v]);
      }
    }
...
export function oracle(id: number, data: GraphData): string {
  const { n, edges = [] } = data,
    base = baseFor(id);
  if (id === 4 || id === 5) {
  ...
  if ([6, 7, 8, 9].includes(id)) {
    const graph = adjacency(
      n,
      id === 8 ? edges.map(([u, v]) => [v, u]) : edges,
      id === 8 || (id === 9 && data.type === "D"),
    );
  ...
  if (id === 10) {
    ...
    for (const [op, u, v] of data.operations!) {
      const key = data.type === "D" || u < v ? edgeKey(u, v) : edgeKey(v, u),
        exists = current.has(key);
      if (op === "ADD") {
        answers.push(Number(!exists));
        current.add(key);
      } else if (op === "DEL") {
        answers.push(Number(exists));
        current.delete(key);
  ...
  if (id === 21) {
    const forward = reachable(n, edges),
      missed = forward.indexOf(false);
    if (missed >= 0) return `NO\n1 ${missed + 1}\n`;
    const backward = reachable(
        n,
        edges.map(([u, v]) => [v, u]),
      ),
      missing = backward.indexOf(false);
    return missing >= 0 ? `NO\n${missing + 1} 1\n` : "YES\n";
  }
  ...
  if (id === 19) {
    const adjacent = new Set(edges.flatMap(([u, v]) => [edgeKey(u, v), edgeKey(v, u)])),
