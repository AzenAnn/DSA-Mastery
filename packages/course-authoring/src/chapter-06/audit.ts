...
import type { LoadedProgramLab } from "@dsa/lab-core";
import type { CompileResult } from "@dsa/lab-runner";
...
import { compareOutput, isProgramLab, normalizeNewlines, loadLab, runProcess } from "@dsa/lab-core";
import { compileTarget, packStudent, verifyProgram } from "@dsa/lab-runner";
import matter from "gray-matter";
import { catalog, labName, labPath } from "./catalog.ts";
import {
  adjacency,
  closure,
  edgeKey,
...
import { randomEdges, rng } from "./fixtures.ts";
...
const countLabels = (labels: number[]) => new Set(labels).size;

function checkSmallGraph(id: number, data: GraphData) {
  if (data.n > 80 || id === 10) return;
  const directed = id === 9 ? data.type === "D" : isDirected(id);
  const a = closure(data.n, data.edges, directed);
  if ([8, 17, 21, 22, 23].includes(id))
...
function checkCondensation(data: GraphData, expected: string) {
  ...
  assert.deepEqual(
    edges,
    edges.slice().sort(([a, b], [c, d]) => a - c || b - d),
  );
  const graph = adjacency(
      k,
      edges.map(([u, v]): Edge => [u - 1, v - 1]),
      true,
    ),
...
function invalidInputChecks() {
  ...
  for (const [id, input] of invalid) assert.throws(() => parseInput(id, input), `invalid input accepted for ${id}`);
}

function randomData(id: number, iteration: number): GraphData {
  const next = rng(id * 100003 + iteration * 997),
    n = (id === 4 ? 3 : [10, 11, 13].includes(id) ? 2 : 1) + next(10);
  const type = iteration % 2 ? "D" : "U",
    directed = id === 9 ? type === "D" : isDirected(id);
  const loops = [8, 13, 17, 21, 22, 23].includes(id);
  let edges = randomEdges(n, next(n * n), id * 9973 + iteration, directed, loops);
  const data: GraphData = { n, edges, type, format: ["E", "M", "L"][Math.floor(iteration / 2) % 3] };
  if (id === 4) {
    const center = next(n);
    data.edges = Array.from({ length: n }, (_, v) => v)
      .filter((v) => v !== center)
      .map((v): Edge => (next(2) ? [center, v] : [v, center]))
      .reverse();
  }
  if (id === 5 && iteration % 2 === 0) {
    ...
    for (let u = 0; u < n; ++u) {
      if (u !== judge) data.edges.push([u, judge]);
    }
  }
  if (id === 6) data.queries = Array.from({ length: 10 }, (): Edge => [next(n), next(n)]);
  if (id === 10) {
    data.edges = [];
  ...
  if (id === 12) {
    data.source = next(n);
    data.destination = next(n);
  }
  if (id === 13 && !edges.length) data.edges = [[0, 0]];
  if ([8, 17, 21, 22, 23].includes(id) && iteration % 3 === 0 && edges.length) {
    edges = [...edges, ...edges.slice(0, 3)];
    data.edges = edges;
  }
  return data;
}

async function differential(item: CatalogItem, lab: LoadedProgramLab, existingCompilation?: CompileResult) {
  ...
  for (let i = 0; i < iterations; ++i) {
    const input = serialize(item.id, randomData(item.id, i)),
      data = parseInput(item.id, input);
    ...
    assert(
      !execution.spawnError && !execution.timedOut && !execution.outputExceeded && execution.code === 0,
      `${lab.labId} random ${i}: execution failed: ${execution.stderr}`,
    );
    assert(
      compareOutput(expected, execution.stdout, { mode: "tokens" }).equal,
      `${lab.labId} random ${i}\n${input}\nExpected:\n${expected}\nActual:\n${execution.stdout}`,
    );
  }
  return iterations;
...
async function checkPack(lab: LoadedProgramLab) {
  ...
  try {
    ...
    for (const command of ["validate", "score"]) {
      const execution = await runProcess(process.execPath, ["packages/lab-cli/dist/cli.js", command, ".", "--json"], {
        cwd: detached,
        timeMs: 120000,
        outputKb: 16384,
      });
      assert.equal(
        execution.code,
        command === "validate" ? 0 : 1,
        `${lab.labId} detached ${command}: ${execution.stderr}`,
      );
      const report = JSON.parse(execution.stdout) as {
        ok: boolean;
        result: { compilation: { ok: boolean }; score: number; maxScore: number };
      };
...
const selected = selectedText === undefined ? catalog.map((item) => item.id) : selectedText.split(",").map(Number);
assert(
  selected.every((id) => catalog.some((item) => item.id === id)),
  "unknown Lab in --only",
);
...
const summary = {
  ...
  cases: 0,
  differentialCases: 0,
  detachedPacks: [] as string[],
};

for (const item of catalog.filter((item) => selected.includes(item.id))) {
  const directory = path.join(root, labPath(item)),
    lab = await loadLab(directory),
    number = String(item.id).padStart(2, "0");
  assert(isProgramLab(lab), `${directory}: expected a program Lab`);
  ...
  for (const testCase of lab.cases) {
    const input = normalizeNewlines(read(path.join(directory, testCase.input))),
      expected = read(path.join(directory, testCase.expected));
    const data = parseInput(item.id, input),
      computed = oracle(item.id, data);
  ...
  const record: LabRecord = { id: lab.labId, cases: 20, maximumOutputBytes };
  if (flags.has("--verify")) {
    const result = await verifyProgram(lab);
    assert(
      result.ok,
      `${lab.labId}: ${JSON.stringify({ checks: result.checks, drift: result.drift, failed: result.solution.cases.filter((entry) => entry.verdict !== "AC") })}`,
    );
  ...
  if (flags.has("--pack") && [9, 18, 23].includes(item.id)) {
    await checkPack(lab);
    summary.detachedPacks.push(lab.labId);
  }
  summary.labs.push(record);
  console.log(
    `PASS ${lab.labId}: 20 cases${record.checks ? `, solution ${record.solutionScore}/100, student ${record.studentScore}/100` : ""}${record.differentialCases === undefined ? "" : `, ${record.differentialCases} random cases`}`,
  );
}
...
const suffix = [summary.compiler, selectedText ?? "all", flags.has("--pack") ? "pack" : ""]
  .filter(Boolean)
  .join("-")
  .replace(/[^\w-]/g, "_");
...
console.log(
  `Ch6 PASS: ${summary.labs.length} Labs, ${summary.cases} fixtures, ${summary.differentialCases} random cases, ${summary.detachedPacks.length} detached packs. Report: ${path.relative(root, reportPath)}`,
);
