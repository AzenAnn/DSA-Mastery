...
import { THIN_MAKEFILE } from "@dsa/lab-core";
import { catalog, labPath } from "./catalog.ts";
import { oracle, parseInput, serialize } from "./contracts.ts";
...
export function renderReadme(item: CatalogItem, input: string, output: string): string {
  const number = String(item.id).padStart(2, "0"),
    title = `Lab 06-E-${number}：${item.title}`,
    relative = labPath(item);
  return `---
...
export function artifactsFor(item: CatalogItem): Map<string, string> {
  ...
  for (const fixture of fixturesFor(item.id)) {
    ...
    const inputPath = `tests/${fixture.caseId}.in`,
      outputPath = `tests/${fixture.caseId}.out`;
    ...
    `${JSON.stringify(
      {
        ...
        judge: {
          ...
          compare: { mode: "tokens" },
          limits: { timeMs: 2000, outputKb: [6, 7, 8, 9].includes(item.id) ? 16384 : 4096 },
        },
      },
      null,
      2,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  for (const item of catalog) {
    const directory = path.join(root, labPath(item)),
      files = artifactsFor(item);
    for (const target of ["student", "solution"])
      assert(fs.existsSync(path.join(directory, target, "main.cpp")), `${directory}: missing ${target} source`);
    if (write) {
      const previousCasesPath = path.join(directory, "tests/cases.json");
      if (fs.existsSync(previousCasesPath)) {
        const previous = JSON.parse(fs.readFileSync(previousCasesPath, "utf8")) as JudgeCase[];
        for (const entry of previous) {
          for (const relative of [entry.input, entry.expected]) {
            // Only retire fixtures formerly listed in this Lab's own manifest.
            assert(/^tests\/\d{3}-[a-zA-Z0-9-]+\.(?:in|out)$/.test(relative));
            if (!files.has(relative) && fs.existsSync(path.join(directory, relative)))
              fs.unlinkSync(path.join(directory, relative));
          }
        }
      }
