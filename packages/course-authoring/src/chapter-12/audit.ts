...
interface LoadedCase extends JudgeCase {
  inputText: string;
  outputText: string;
}
...
function labDirectory(sequence: number) {
...
function loadLab(sequence: number): LoadedCase[] {
  ...
  const cases = JSON.parse(read(path.join(directory, "tests", "cases.json"))) as JudgeCase[];
...
function wFunction(a: number, b: number, c: number, memo = new Map<string, number>()): number {
  ...
  const answer =
    a < b && b < c
      ...
      : wFunction(a - 1, b, c, memo) +
        wFunction(a - 1, b - 1, c, memo) +
        wFunction(a - 1, b, c - 1, memo) -
        wFunction(a - 1, b - 1, c - 1, memo);
  memo.set(key, answer);
  return answer;
}

function checkFunction(cases: LoadedCase[]) {
...
function encodePower(value: number): string {
  const terms: string[] = [];
...
function checkPowerExpression(cases: LoadedCase[]) {
  for (const item of cases) {
...
function buildTotem(level: number): string[] {
...
function checkTotem(cases: LoadedCase[]) {
  for (const item of cases) {
    const n = Number(tokens(item.inputText)[0]);
...
function checkFastPower(cases: LoadedCase[]) {
  for (const item of cases) {
...
function expectedPardon(level: number) {
  const size = 2 ** level;
  return Array.from({ length: size }, (_, row) =>
    Array.from({ length: size }, (_, column) => ((row | column) === size - 1 ? 1 : 0)),
  );
}

function checkPardon(cases: LoadedCase[]) {
  for (const item of cases) {
    const n = Number(tokens(item.inputText)[0]);
...
function locateCowCode(initial: string, rawPosition: string) {
...
function checkMergeSort(cases: LoadedCase[]) {
  for (const item of cases) {
    const values = parseArrayCase(item.inputText);
    assert.deepEqual(
      tokens(item.outputText).map(Number),
...
const carpetCells: Record<number, [number, number][]> = {
  1: [
    [0, 0],
    [-1, 0],
    [0, -1],
...
function checkInversions(cases: LoadedCase[]) {
  for (const item of cases) {
    const values = parseArrayCase(item.inputText);
...
function expressionResults(expression: string, memo = new Map<string, number[]>()): number[] {
...
function checkExpressions(cases: LoadedCase[]) {
  for (const item of cases) {
...
function checkBeautiful(cases: LoadedCase[]) {
  for (const item of cases) {
    const n = Number(tokens(item.inputText)[0]);
...
function checkReversePairs(cases: LoadedCase[]) {
  for (const item of cases) {
    const values = parseArrayCase(item.inputText);
    if (values.length > 300) continue;
    let answer = 0;
...
function checkSmaller(cases: LoadedCase[]) {
  for (const item of cases) {
    const values = parseArrayCase(item.inputText);
    if (values.length > 300) continue;
...
function checkRangeSum(cases: LoadedCase[]) {
  for (const item of cases) {
    const input = tokens(item.inputText).map(Number);
