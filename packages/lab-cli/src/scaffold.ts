import type { LabType } from "@dsa/lab-core";
...
import { allocateLabIdentity } from "@dsa/lab-runner";
...
function readme({
  ...
  const title = `${formatLabDocumentTitlePrefix(labId)}${names[type]}`;
  const body =
    type === "quiz"
      ? "## 选择题\n\n<QuizSet />"
...
async function syncChapterCategoryDirectories(root: string, chapter: number): Promise<void> {
  ...
  for (const category of LAB_CATEGORIES) {
    ...
    else if (!(await pathExists(marker))) await writeFile(marker, "", { encoding: "utf8", flag: "wx" });
  }
}

async function writeQuizLab(root: string, relativeRoot: string): Promise<void> {
  await write(
    root,
    `${relativeRoot}/lab.json`,
    json({
      $schema: "../../../../schemas/lab.schema.json",
      schemaVersion: 1,
      type: "quiz",
      quiz: { questions: "quiz.json", questionType: "single-choice", reveal: "after-submit", scoring: "points" },
    }),
  );
  await write(
    root,
    `${relativeRoot}/quiz.json`,
    json([
      {
        id: "q1",
        stem: "请替换为题面。",
...
async function writeProgramLab(root: string, relativeRoot: string): Promise<void> {
  await write(
    root,
    `${relativeRoot}/lab.json`,
    json({
...
async function writeProjectLab(root: string, relativeRoot: string): Promise<void> {
  await write(
    root,
    `${relativeRoot}/lab.json`,
    json({
      ...
      buildSystem: "cmake",
      tasks: [
        { id: "implementation", path: "tasks/task-01-implementation", weight: 80, kind: "ctest", dependsOn: [] },
        { id: "report", path: "report", weight: 20, kind: "manual", dependsOn: ["implementation"] },
      ],
    }),
  );
  await write(root, `${relativeRoot}/Makefile`, THIN_MAKEFILE);
  ...
  await write(
    root,
    `${relativeRoot}/CMakePresets.json`,
    json({
      version: 6,
      configurePresets: [
        {
          ...
          cacheVariables: { CMAKE_CXX_STANDARD: "17", CMAKE_CXX_STANDARD_REQUIRED: "ON", LAB_USE_SOLUTION: "OFF" },
        },
        {
          name: "solution",
          binaryDir: "${sourceDir}/.lab-cache/cmake/solution",
          cacheVariables: { CMAKE_CXX_STANDARD: "17", CMAKE_CXX_STANDARD_REQUIRED: "ON", LAB_USE_SOLUTION: "ON" },
        },
      ],
      buildPresets: [
        { name: "student", configurePreset: "student" },
        { name: "solution", configurePreset: "solution" },
      ],
      testPresets: [
        { name: "student", configurePreset: "student", output: { outputOnFailure: true } },
        { name: "solution", configurePreset: "solution", output: { outputOnFailure: true } },
      ],
    }),
  );
  await write(
    root,
    `${relativeRoot}/tasks/task-01-implementation/task.json`,
    json({
      $schema: "../../../../../../schemas/task.schema.json",
      schemaVersion: 1,
      kind: "ctest",
      ctest: { buildTargets: ["implementation"], tests: [{ name: "implementation-smoke", points: 100 }] },
    }),
  );
  await write(
    root,
    `${relativeRoot}/tasks/task-01-implementation/CMakeLists.txt`,
    "set(IMPLEMENTATION_SOURCE student/main.cpp)\nif(LAB_USE_SOLUTION)\n  set(IMPLEMENTATION_SOURCE solution/main.cpp)\nendif()\nadd_executable(implementation ${IMPLEMENTATION_SOURCE})\nadd_test(NAME implementation-smoke COMMAND implementation)\n",
  );
  ...
  await write(
    root,
    `${relativeRoot}/tasks/task-01-implementation/README.md`,
    "# Task 01：Implementation\n\n请补充接口、验收标准和边界情况。\n",
  );
  await write(
    root,
    `${relativeRoot}/report/task.json`,
    json({
      $schema: "../../../../../schemas/task.schema.json",
      schemaVersion: 1,
      kind: "manual",
...
export async function createLab(options: CreateLabOptions, root: string): Promise<CreatedLab> {
  const { type, slug } = options;
  const chapter = Number(options.chapter);
  if (type !== "quiz" && type !== "program" && type !== "project") {
    throw new LabError("ARGUMENT_INVALID", "--type 必须是 quiz、program 或 project");
  }
  if (!Number.isInteger(chapter) || chapter < 0 || chapter > 99) {
    throw new LabError("ARGUMENT_INVALID", "--chapter 必须是 0～99 的整数");
  }
  if (typeof slug !== "string" || !LAB_SLUG_PATTERN.test(slug)) {
    throw new LabError("ARGUMENT_INVALID", "--slug 必须是小写 kebab-case");
  }
  ...
  await write(
    ...
    readme({ chapter, order: identity.order, type, labId: identity.id, relativeRoot }),
  );
  ...
  return { labRoot, relativeRoot, type, labId: identity.id, order: identity.order };
}
