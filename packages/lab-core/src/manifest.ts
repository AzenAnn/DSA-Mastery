...
import type { LabType } from "./layout.ts";
...
import { parseLabId } from "./lab-id.ts";
import { isLabType } from "./layout.ts";
...
export const COMPARE_MODES: ReadonlySet<string> = new Set<CompareMode>(["exact", "tokens", "float"]);
export const TASK_KINDS: ReadonlySet<string> = new Set<TaskKind>(["stdio", "ctest", "manual"]);
...
export async function loadCases(labRoot: string, casesPath: string): Promise<LabCase[]> {
