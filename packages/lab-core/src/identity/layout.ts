...
const LAB_TYPES: ReadonlySet<string> = new Set<LabType>(["quiz", "program", "project"]);
export const LAB_CATEGORIES = ["theory", "exercise", "project"] as const satisfies readonly LabCategory[];

const LAB_TYPE_TO_TAG = { quiz: "T", program: "E", project: "P" } as const satisfies Record<LabType, LabTag>;
const LAB_TYPE_TO_CATEGORY = {
  quiz: "theory",
  program: "exercise",
  project: "project",
} as const satisfies Record<LabType, LabCategory>;
const LAB_CATEGORY_TO_TAG = { theory: "T", exercise: "E", project: "P" } as const satisfies Record<LabCategory, LabTag>;
