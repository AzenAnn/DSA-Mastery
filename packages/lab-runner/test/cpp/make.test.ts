...
import { materializeLab, REPO_ROOT } from "@dsa/lab-testkit";
import { afterAll, beforeAll, describe, expect, it, onTestFinished } from "vitest";

const projectRoot = REPO_ROOT;
// 判题会写 .lab-cache，Lab 先物化到临时目录，这个文件才能和 golden 并行跑。
const programCopy = await materializeLab("labs/chapter-01/exercise/E-01-01-sequential-list-deduplication");
const projectCopy = await materializeLab("labs/chapter-08/project/P-08-01-avl-tree-rotations");
const lab = programCopy.root;
const labRoot = programCopy.root;
...
async function runJson(command: string, args: string[], cwd = projectRoot): Promise<CliReport> {
  ...
  return JSON.parse(result.stdout) as CliReport;
}

function stable(report: CliReport) {
  return {
...
// describe.skipIf 不做类型收窄，探测不到就留空串，跳过时根本执行不到。
const make = await chooseMake();

afterAll(async () => {
  await Promise.all([programCopy.cleanup(), projectCopy.cleanup()]);
});

describe.skipIf(make === "")("Make and CLI stay consistent", () => {
  let cliReport: CliReport;
  ...
  it("make run hides a non-full-score exit status while make score stays strict", async () => {
    ...
    expect(
      (JSON.parse(learnerRun.stdout) as CliReport).result.score,
      "starter should remain non-full under make run",
    ).toBeLessThan(100);
    expect(
      `${learnerRun.stdout}\n${learnerRun.stderr}`,
