...
import { runProcess, STANDALONE_CLI_FILENAME } from "@dsa/lab-core";
import { packStudent, scoreProject, projectStatus } from "@dsa/lab-runner";
import { loadProjectLab, materializeLab, REPO_ROOT } from "@dsa/lab-testkit";
import { expect, it, onTestFinished } from "vitest";

const SOURCE_LAB = "labs/chapter-02/project/P-02-04-expression-evaluator";
const source = path.join(REPO_ROOT, SOURCE_LAB);
const modules = ["stack", "tokenizer", "postfix", "evaluator"];
const relative = (index: number) => `tasks/task-0${index + 1}-${index === 3 ? "final" : modules[index]}`;
const statuses = (result: { tasks: { status?: string }[] }) => result.tasks.map((task) => task.status);

it(
  "real expression project: reference, starter, independent builds, dependency faults, integration and external pack",
  { timeout: 300_000 },
  async () => {
    ...
    await writeFile(
      unitFile,
      unitOriginal.replace('CHECK(argc == 2, "test group");', 'CHECK(false, "injected unit failure");'),
    );
    ...
    // 打包要用未被本用例改动过的原始 Lab，但不能碰仓库，所以再物化一份。
    // 打包要用未被本用例改动过的原始 Lab，但不能碰仓库，所以再物化一份。
    const pristine = await materializeLab(SOURCE_LAB);
    onTestFinished(pristine.cleanup);
    const packed = await packStudent(await loadProjectLab(pristine.root));
    ...
    for (const command of ["validate", "score"]) {
      const run = await runProcess(process.execPath, [STANDALONE_CLI_FILENAME, command, ".", "--json"], {
        cwd: external,
        timeMs: 120_000,
        outputKb: 4096,
      });
