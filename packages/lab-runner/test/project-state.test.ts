import type { ProjectState, ProjectStateLab } from "@dsa/lab-runner";
import { mkdir, mkdtemp, readFile, rm, writeFile } from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { currentProject, projectInputs, withProjectLock } from "@dsa/lab-runner";
import { expect, it, onTestFinished } from "vitest";

async function fixture(): Promise<ProjectStateLab> {
  const labRoot = await mkdtemp(path.join(os.tmpdir(), "dsa project state "));
  onTestFinished(() => rm(labRoot, { recursive: true, force: true }));
  const declarations: Omit<ProjectStateLab["tasks"][number], "config">[] = [
  ...
  const tasks: ProjectStateLab["tasks"] = declarations.map((task) => ({
    ...task,
    config: { schemaVersion: 1, kind: task.kind, checklist: ["Review"] },
  }));
...
it("project lock rejects concurrent grading and releases on failure", async () => {
  ...
  await withProjectLock(lab, async () => {
    const lock = JSON.parse(await readFile(path.join(lab.labRoot, ".lab-cache/project.lock"), "utf8")) as {
      pid: number;
    };
    expect(lock.pid).toBe(process.pid);
    await expect(withProjectLock(lab, async () => {})).rejects.toThrow(
      expect.objectContaining({ code: "PROJECT_BUSY" }),
    );
  });
  await expect(
    withProjectLock(lab, async () => {
