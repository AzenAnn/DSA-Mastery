import type { MsvcEnvironment } from "./toolchain.ts";
import type { LoadedLab, Runner, Version } from "@dsa/lab-core";
import process from "node:process";
import { compareVersion, formatVersion, LabError, MINIMUMS, parseVersion, runProcess } from "@dsa/lab-core";
import { createMsvcEnvironment } from "./toolchain.ts";
...
export async function inspectEnvironment(lab: LoadedLab, options: InspectOptions = {}): Promise<EnvironmentReport> {
  ...
  // macOS 的 g++ 其实是 clang 驱动，按 Clang 的下限判定才准确。
  ...
  const issues: string[] = [];
  if (lab.manifest.type !== "quiz" && !compilers.some((item) => item.available && item.meetsMinimum)) {
    issues.push("需要 GCC >= 11、Clang >= 14 或 Visual Studio 2022 / MSVC >= 19.30 之一");
  }
  if (lab.manifest.type === "project" && !(cmake.available && cmake.meetsMinimum)) {
    issues.push("Project Lab 需要 CMake >= 3.25");
  }
  ...
  return {
    ...
    standard: lab.manifest.type === "quiz" ? undefined : lab.manifest.toolchain.standard,
    tools: probes,
    makeOptional: true,
...
export function assertEnvironmentReady(environment: EnvironmentReport): void {
