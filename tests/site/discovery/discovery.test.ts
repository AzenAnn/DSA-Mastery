...
import { auditPages, loadContentCorpus, normalizeBase, readSiteArtifact } from "@dsa/course-index";
import { REPO_ROOT } from "@dsa/lab-testkit";
import { afterAll, beforeAll, expect, it } from "vitest";

const projectRoot = REPO_ROOT;
// 自动发现要往 content/ 和 labs/ 塞临时教材，产物必须和部署用的 dist/pages 分开。
const distPages = path.join(projectRoot, "dist", "discovery-pages");
...
/** 用和站点产物审计同一个构建入口，只是换一个 outDir。 */
function buildDiscoverySite(): void {
  const result = spawnSync(
    process.execPath,
    ["--experimental-strip-types", "--no-warnings", path.join(projectRoot, "tests/support/vitepress-build.entry.ts")],
    {
      cwd: projectRoot,
      env: {
        ...pagesEnvironment,
        VP_ROOT: projectRoot,
        VP_OUT: distPages,
        VP_BASE: pagesEnvironment.GITHUB_PAGES_BASE_PATH,
      },
      encoding: "utf8",
      stdio: "pipe",
    },
  ...
  if (result.status !== 0) {
    throw new Error(
      ["自动发现的 VitePress 构建失败", result.error?.message, result.stdout, result.stderr].filter(Boolean).join("\n"),
    );
  }
}

// 仅允许删除本套件自己创建的 fixture 目录，避免清理阶段误删教材。
function assertFixtureTarget(target: string, allowedParent: string, expectedName: string) {
...
const html: Record<"lesson" | "lab" | "sidebar" | "stableTitle" | "chapterFiveOutline", string> = {} as never;
let searchableJavaScript: string;
...
beforeAll(async () => {
  ...
  // 内容规则与产物审计都在进程内跑，只有 VitePress 构建仍需独立进程。
  const corpus = await loadContentCorpus(projectRoot);
  expect(corpus.violations, "临时教材与 Lab 应当直接通过全部内容规则").toEqual([]);
  buildDiscoverySite();
  const artifact = await readSiteArtifact(
    projectRoot,
    distPages,
    normalizeBase(pagesEnvironment.GITHUB_PAGES_BASE_PATH),
  );
  await auditPages(artifact);
