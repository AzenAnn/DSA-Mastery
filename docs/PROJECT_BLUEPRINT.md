...
> 状态：Demo / MVP 讨论稿  
> 适用对象：两名核心维护者及后续贡献者  
> 核心判断：先验证“能否持续产出高质量章节”，再扩展出版与交互能力。

## 1. 项目定位

### 一句话简介

DSA Lab 是一套在真实学习过程中共同写成、实现并验证的数据结构与算法开源教材。

### 项目介绍

DSA Lab 不把“看完课程”当作掌握，而要求维护者把每个知识点解释清楚、实现出来、用实验验证，并接受同伴 Review。仓库同时记录教材正文、实验任务和网站展示；每一次章节更新都是一次完整的学习交付。

项目面向正在学习数据结构与算法的学生。内容从课程主线出发，逐步覆盖概念、ADT、算法、复杂度、实现、测试、常见错误、练习和扩展阅读。最终可以成长为在线教材、可运行代码库、实验体系和交互式算法 Playground，但 Demo 只验证最小闭环。
...
- 学习价值优先于关注度，正确性优先于更新速度。
- 如果不能解释、实现、测试和教给别人，就还没有真正掌握。
- 工程化服务于学习；两个人能长期执行，比功能齐全更重要。
- AI 是辅助者和质检线索提供者，人工对知识、引用和发布负责。
- Markdown 是内容单一事实来源，避免在多个格式中重复修改。

### 项目目标

1. 建立一套由浅入深、术语统一、可追溯的数据结构与算法教材。
2. 为核心结构和算法提供真正可运行、可测试的实现与实验。
3. 让两位维护者都经历研究、写作、编程、测试、Review 和发布。
4. 用自动渲染的网站降低发布成本，保持内容与页面同步。
5. 形成一个至少可持续一学期、能够逐步接受外部贡献的工作流。
...
1. 范围、学习目标、前置知识明确，`status` 与完成度一致。
...
1. 新增一篇 Markdown 是否真的无需改导航代码？
2. 当前 frontmatter 字段是否足够且易维护？
3. 一章拆成几篇最适合写作和阅读？
4. 两人 Review 的时间成本是否可持续？
5. 下一个月最值得做的是新内容、测试还是网站体验？
