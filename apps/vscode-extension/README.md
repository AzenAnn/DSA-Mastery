...
必须**把 `apps/vscode-extension/` 作为工作区根目录打开**，再按 <kbd>F5</kbd>：
...
扩展校验判题报告的 `reportVersion`，当前支持 `1`。如果 `packages/lab-cli/dist/cli.js` 的报告格式变了，扩展会提示需要升级而不是静默出错。改动内核的报告结构时，记得同步 `cli.ts` 里的 `SUPPORTED_REPORT_VERSION` 和相关类型。
