import type { Chapter, LabEntry, ProjectLab } from "../labs/discovery";
import type { ProjectProgress } from "../progress/project";
import type { LabProgress, ProgressTracker, QuizProgress } from "../progress/tracker";
import { formatNumber } from "@dsa/lab-core";
import * as vscode from "vscode";
import { discoverProgramLabs } from "../labs/discovery";
import { shortLabTitle } from "../labs/identity";
...
function buildQuizTooltip(
  lab: Extract<LabEntry, { type: "quiz" }>,
  state: QuizProgress | undefined,
...
function buildTooltip(
  lab: Extract<LabEntry, { type: "program" }>,
  state: LabProgress | undefined,
): vscode.MarkdownString {
  const lines = [`**${lab.id} · ${lab.title}**`, ""];
  if (lab.description) lines.push(lab.description, "");

  const meta = [
    (lab.difficulty ?? "") === "" ? "" : `难度：${lab.difficulty}`,
    (lab.duration ?? "") === "" ? "" : `预计：${lab.duration}`,
  ]
    .filter((part) => part !== "")
    .join(" · ");
  ...
  if (!state || state.submissionCount === 0) {
    ...
    if (state.lastSubmission) {
      ...
      lines.push(`最近一次：${last.verdict} ${last.score}/${last.maxScore}（${formatTime(last.at)}）`);
    }
  }
...
function describeProjectState(state: ProjectProgress | undefined): string {
  ...
  if (state?.current?.tasks.some((task) => task.status === "STALE"))
    return `${formatNumber(state.automatedScore)}/${formatNumber(state.automatedMax)} · 需要重测`;
  if (!state || state.submissionCount === 0) return "";
  if (state.internalError) return "评测内部错误";
  if (projectProgressPassed(state)) return `${formatNumber(state.automatedScore)}/${formatNumber(state.automatedMax)}`;
  if (state.automatedFull && state.manualPending > 0) return "自动通过 · 待人工";
  return `${formatNumber(state.automatedScore)}/${formatNumber(state.automatedMax)} · 待人工 ${formatNumber(state.manualPending)}`;
}
...
function buildProjectTooltip(lab: ProjectLab, state: ProjectProgress | undefined): vscode.MarkdownString {
  ...
  if (!state || state.submissionCount === 0) {
    ...
    lines.push(
      status,
      `自动得分：${formatNumber(state.automatedScore)}/${formatNumber(state.automatedMax)}`,
      `待人工：${formatNumber(state.manualPending)}`,
      `提交次数：${state.submissionCount}`,
    );
