import type { ProjectCurrentState, ProjectCurrentTask, ProjectScoreResult, ScoreResult } from "../cli/client";
import type { LoadedTestCase, ProjectLab, ProjectTask, ProgramLab, QuizLab } from "../labs/discovery";
import type { ProjectProgress, ProjectSubmissionSummary, ProjectTaskSubmissionSummary } from "../progress/project";
import type { LabProgress, QuizProgress } from "../progress/tracker";
...
export function renderQuizPanelHtml(
  ...
  const questionHtml = questions
    .map((question, index) => renderQuizQuestion(question, index, quizProgress?.answers[question.id]))
    .join("\n");
...
<link rel="stylesheet" href="${katexCssUri.toString()}" /><link rel="stylesheet" href="${styleUri.toString()}" /></head><body>
...
export function renderProjectPanelHtml(options: ProjectPanelOptions): string {
  ...
  const taskHtml = lab.tasks
    .map((task) =>
      renderProjectTask(
        task,
        lab.studentFiles.filter((file) => file.taskId === task.id),
      ),
    )
    .join("\n");
...
<div class="lab-workspace project-workspace${initialInspectorOpen ? "" : " is-inspector-collapsed"}">
  <main class="lab-reading-surface" aria-label="Project 题面与任务">
    ...
    <div class="project-support-files">${(lab.supportFiles ?? [])
      .filter(
        (file) =>
          file === "CMakeLists.txt" ||
      ...
      .map(
        (file) =>
          `<button class="project-file-button" data-project-file="${escapeHtml(file)}" type="button">${escapeHtml(file)}</button>`,
      )
      .join("")}</div>
  ...
  const fileHtml =
    files.length === 0
      ...
      : files
          .map(
            (file) =>
              `<button type="button" class="project-file-button" data-project-file="${escapeHtml(file.relativePath)}"><code>${escapeHtml(file.relativePath)}</code><span>打开</span></button>`,
          )
          .join("");
  let details = "";
  if (task.kind === "stdio") {
...
function renderProjectResultTask(task: ProjectTaskSubmissionSummary): string {
  ...
  if (task.kind === "stdio") {
    ...
    const build = task.buildFailed
      ...
      : "";
    nested = `${build}${(task.blockedBy?.length ?? 0) === 0 ? "" : `<p>依赖构建阻塞：${escapeHtml(task.blockedBy!.join(", "))}</p>`}${(task.diagnostic ?? "") === "" ? "" : `<pre class="diagnostic project-diagnostic">${escapeHtml(task.diagnostic!)}</pre>`}${rows ? `<table class="cases-table project-result-table"><thead><tr><th>CTest</th><th>结果</th><th>耗时</th><th>得分</th></tr></thead><tbody>${rows}</tbody></table>` : ""}`;
  } else {
...
function currentTaskSummary(task: ProjectCurrentTask): ProjectTaskSubmissionSummary {
  ...
  return {
    ...task,
    cases: task.judge?.cases,
    buildFailed: task.build ? !task.build.ok : undefined,
    buildPhase: task.build?.phase,
    diagnostic: task.judge
      ? task.judge.compilation.stderr || task.judge.compilation.stdout
      : task.build && !task.build.ok
        ? `${build?.stdout ?? ""}\n${build?.stderr ?? ""}`
        : undefined,
  };
}
...
export function renderProjectCurrentHtml(current?: ProjectCurrentState): string {
  if (!current)
    return '<p class="project-current-unknown">当前代码尚未核验。<button type="button" data-project-refresh>刷新状态</button></p>';
  ...
  const heading = current.complete
    ? "Project 完成"
    : current.automatedFull && current.manualPending
      ? "自动通过 · 待人工"
      : "当前工程未完成";
...
export function renderProjectResultHtml(result: ProjectScoreResult, progress: ProjectProgress): string {
  if (result.current)
    return `<p>本次测评：${escapeHtml(result.selectedTaskId ?? "整个 Project")} · 第 ${progress.submissionCount} 次提交</p>${renderCurrentDetails(result.current)}`;
  return renderProjectResultSummary(
    {
      ...result,
      ...
      tasks: result.tasks.map((task) => {
        ...
        if (task.kind === "stdio") {
          return {
            ...task,
            cases: task.judge.cases.map((item) => ({
              id: item.id,
...
function renderStoredResult(progress: LabProgress): string {
  ...
  <p>${verdictLabel(last.verdict)} · ${last.score}/${last.maxScore}${progress.passed ? " · 本题已通过" : ""}</p>
