import type { ActivityEvent, ChapterBar, Counters, Heatmap, TrendPoint } from "../progress/stats";
import { getRankProgress } from "../progress/rank";
import { buildHeatmap, buildTrend, countActivity, localDateKey } from "../progress/stats";
...
<section class="stats-block stats-activity" aria-labelledby="stats-activity-title">
  ...
  ${maps
    .map(
      (map) => `<div class="year-pane" data-year="${map.year}"${map.year === currentYear ? "" : " hidden"}>
...
function renderHeatmap(map: Heatmap, other: Heatmap, metric: "submit" | "pass", today: string): string {
  ...
  const focusIndex =
    preferredIndex >= 0
      ...
      : Math.max(
          0,
          map.cells.findLastIndex((cell) => cell.count > 0),
        );
  ...
  const weekdayLabels = ["日", "一", "二", "三", "四", "五", "六"]
    ...
    .join("");
  const legend = HEAT_COLORS.map(
    (color, index) => `<rect x="${index * (CELL + GAP)}" width="${CELL}" height="${CELL}" rx="2" fill="${color}" />`,
  ).join("");
...
function renderTrend(points: TrendPoint[]): string {
  ...
  const coords = points
    .map((point) => `${xFor(point.date).toFixed(1)},${yFor(point.cumulativePasses).toFixed(1)}`)
    .join(" ");
  const ticks = [...new Set([0, 0.25, 0.5, 0.75, 1].map((ratio) => Math.round(max * ratio)))];
  const tickMarkup = ticks
    .map(
      (value) => `<g class="trend-tick" data-value="${value}">
