import type { ProjectScoreResult, ScoreResult } from "../cli/client";
import type { EnvironmentGuard } from "../cli/doctor";
import type { LabEntry, ProjectLab } from "../labs/discovery";
import type { ProgressTracker } from "../progress/tracker";
import type { PanelNav, QuizQuestionView } from "./panel-html";
import { mkdtemp, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import path from "node:path";
import process from "node:process";
import * as vscode from "vscode";
import { CliError, readProjectCurrent, scoreLab, scoreProject } from "../cli/client";
import { collectProjectFiles, loadTestCases, studentSourcePath } from "../labs/discovery";
import { renderMarkdownFragment, renderReadme } from "./markdown";
import {
  renderCurrentDetails,
  renderPanelHtml,
  renderProjectCurrentHtml,
  renderProjectHeader,
  renderProjectPanelHtml,
  renderProjectResultHtml,
  renderQuizFeedbackHtml,
  renderQuizPanelHtml,
  renderResultHtml,
} from "./panel-html";
...
export class LabPanel {
  ...
  static async show(lab: LabEntry, deps: PanelDeps): Promise<void> {
    ...
    if (LabPanel.current.submitting || LabPanel.current.quizBatchInProgress) {
  ...
  static refreshCurrent(labId: string): void {
    ...
    void panel.panel.webview.postMessage({
      type: "projectCurrent",
      html: renderProjectCurrentHtml(progress?.current),
      headerHtml: renderProjectHeader(panel.lab, progress),
      detailsHtml: progress?.current ? renderCurrentDetails(progress.current) : undefined,
    });
  }
  ...
  private async load(lab: LabEntry): Promise<void> {
    ...
    if (lab.type === "quiz") {
      ...
      this.quizViews = lab.quizQuestions.map((question) => ({
        ...
        hintHtml:
          (question.hint ?? "") === "" ? undefined : renderMarkdownFragment(question.hint!, lab, this.panel.webview),
    ...
    if (lab.type === "project") {
      if (vscode.workspace.isTrusted) {
        try {
          ...
          this.deps.progress.invalidateProjectCurrent(lab.id);
        }
      }
      if (version !== this.loadVersion || this.disposed) return;
  ...
  private navFor(lab: LabEntry): PanelNav {
    ...
    const at = (offset: number) => {
      ...
      return target === undefined ? undefined : { name: target.id, title: target.title };
    };
      ...
      case "openProjectReadme":
        if (this.lab.type === "project" && this.lab.tasks.some((task) => task.readmePath === message.filePath)) {
          await vscode.commands.executeCommand(
            ...
            vscode.Uri.file(path.join(this.lab.labPath, message.filePath!)),
          );
        }
        return;
      case "openProjectReport":
        if (this.lab.type === "project") {
          const document = await vscode.workspace.openTextDocument(
            path.join(this.lab.labPath, ".lab-cache", "project-results-student.json"),
          );
  ...
  private async answerQuiz(questionId?: string, selected?: number): Promise<void> {
    if (this.lab.type !== "quiz" || questionId === undefined || selected === undefined || !Number.isInteger(selected)) {
      return;
    }
  ...
  private async navigate(labName?: string): Promise<void> {
    if (labName === undefined || this.submitting || this.quizBatchInProgress) return;
    const target = this.deps
      .siblings()
      .find((item) => item.id === labName || item.name === labName || item.legacyNames.includes(labName));
    if (!target) return;
  ...
  private async openSource(filePath?: string): Promise<void> {
    ...
    if (this.lab.type === "project") {
      if (filePath !== undefined && (await collectProjectFiles(this.lab.labPath)).includes(filePath)) {
        const document = await vscode.workspace.openTextDocument(path.join(this.lab.labPath, filePath));
        await vscode.window.showTextDocument(document, vscode.ViewColumn.Beside);
        return;
      }
      let studentFile =
        filePath === undefined ? undefined : this.lab.studentFiles.find((file) => file.relativePath === filePath);
      if (!studentFile) {
        if (this.lab.studentFiles.length === 1) {
          ...
          studentFile = picked
            ? this.lab.studentFiles.find((file) => file.relativePath === picked.filePath)
            : undefined;
        }
      }
  ...
  private async submit(taskId?: string): Promise<void> {
    ...
    if (
      taskId !== undefined &&
      (lab.type !== "project" || !lab.tasks.some((task) => task.id === taskId && task.kind !== "manual"))
    ) {
      return;
    }
    ...
    const post = (message: object) => {
      if (!this.disposed && this.loadVersion === version && this.lab.id === lab.id)
    ...
    try {
      ...
      if (lab.type === "program") {
        ...
        const open = vscode.workspace.textDocuments.find(
          (doc) => comparablePath(doc.fileName) === comparablePath(sourcePath),
        );
  ...
  private async saveProjectFiles(lab: ProjectLab): Promise<void> {
    const allowed = new Set(
      (await collectProjectFiles(lab.labPath)).map((file) => comparablePath(path.join(lab.labPath, file))),
    );
    const dirtyDocuments = vscode.workspace.textDocuments.filter(
      (document) => document.isDirty && allowed.has(comparablePath(document.fileName)),
    );
    for (const document of dirtyDocuments) {
      if (!(await document.save()))
        throw new CliError(`保存失败：${document.fileName}。本次测评已取消。`, "SAVE_FAILED");
    }
    if (
      ...
      throw new CliError("保存期间 Project 输入再次变化，请完成编辑后重试。", "INPUT_CHANGED");
    }
  }
  ...
  private async notify(result: ScoreResult): Promise<void> {
    if (result.verdict === "AC" && result.score === result.maxScore) {
      void vscode.window.showInformationMessage(`通过：${this.lab.title}（${result.score}/${result.maxScore}）`);
      return;
    }
    ...
    if (choice !== undefined && failed !== undefined) await this.openDiff(failed.id);
  }
  ...
  private async notifyProject(result: ProjectScoreResult, title: string): Promise<void> {
    ...
    if (failed === undefined && result.selectedTaskId !== undefined) {
      await vscode.window.showInformationMessage(
...
export function projectDirtyFiles(lab: ProjectLab): string[] {
  return vscode.workspace.textDocuments
    ...
    .map((document) => path.relative(lab.labPath, document.fileName).split(path.sep).join("/"))
    .filter(
      (file) =>
        file !== "" &&
        !file.startsWith("../") &&
        !path.isAbsolute(file) &&
        !file.split("/").some((part) => ["solution", ".lab-cache", ".git", "node_modules"].includes(part)),
    );
}
