...
import { parseQuizQuestions } from "@dsa/lab-core";
import matter from "gray-matter";
import { ch04LegacyNames } from "../progress/migrations/ch04";
import { ch07LegacyNames } from "../progress/migrations/ch07";
import { readStableLabId } from "./identity";
...
type ProjectTaskKind = "stdio" | "ctest" | "manual";

interface ProjectCtestTest {
  name: string;
  points: number;
}
...
async function loadProjectLab(
  ...
  for (const candidate of manifest.tasks) {
    ...
    const weight = typeof rawEntry.weight === "number" ? rawEntry.weight : Number.NaN;
    ...
    if (
      rawEntry.buildDependsOn !== undefined &&
      (!buildDependsOn || new Set(buildDependsOn).size !== buildDependsOn.length)
    ) {
      return undefined;
    }
    ...
    if (await isWithinRealDirectory(labPath, path.join(taskPath, "README.md")))
      projectTask.readmePath = `${projectTask.relativePath}/README.md`;
    ...
    if (kind === "stdio") {
      ...
      for (const rawTest of rawTests) {
        if (rawTest === null || typeof rawTest !== "object" || Array.isArray(rawTest)) return undefined;
      ...
      for (const key of ["buildTargets", "moduleTargets"] as const) {
        ...
        if (
          ...
          names.some((name) => !/^\w[\w.+-]*$/iu.test(name))
        ) {
          return undefined;
        }
  ...
  for (const task of tasks) {
    if (
      [...task.dependsOn, ...(task.buildDependsOn ?? [])].some(
        (dependency) => !taskIds.has(dependency) || dependency === task.id,
      )
    ) {
      return undefined;
    }
...
function buildLab(
  ...
  front: Record<string, unknown>,
  details:
    | { type: "program"; studentSources: string[]; casesFile: string }
    | { type: "quiz"; quizQuestions: QuizQuestion[] },
): ProgramLab | QuizLab {
...
export async function loadTestCases(lab: ProgramLab): Promise<LoadedTestCase[]> {
...
async function loadTestCasesAt(root: string, casesFile: string): Promise<LoadedTestCase[]> {
  ...
  for (const testCase of cases) {
    ...
    const [inputText, expectedText] = await Promise.all([
      isSafeRelativePath(testCase.input)
        ? readFile(path.join(root, testCase.input), "utf8").catch(() => "")
        : Promise.resolve(""),
      isSafeRelativePath(testCase.expected)
        ? readFile(path.join(root, testCase.expected), "utf8").catch(() => "")
        : Promise.resolve(""),
    ]);
