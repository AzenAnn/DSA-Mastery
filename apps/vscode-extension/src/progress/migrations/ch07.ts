import type { LabKeyAlias } from "../keys";
...
export const CH07_RENUMBERING = [
  [1, 1, "dfs-timestamps"],
  [5, 2, "iterative-dfs"],
  [13, 3, "dfs-edge-classification"],
  [14, 4, "eulerian-classification"],
  [15, 5, "seven-bridges"],
  [16, 6, "course-schedule"],
  [17, 7, "course-schedule-ii"],
  [18, 8, "eventual-safe-states"],
  [19, 9, "food-chain-count"],
  [20, 10, "parallel-courses"],
  [21, 11, "critical-path"],
  [4, 12, "minimum-spanning-tree"],
  [22, 13, "connect-cities"],
  [23, 14, "connect-points"],
  [24, 15, "minimum-effort-path"],
  [7, 16, "dijkstra-trace"],
  [8, 17, "dijkstra-matrix-path"],
  [9, 18, "network-delay-time"],
  [11, 19, "bellman-ford-negative"],
  [12, 20, "floyd-all-pairs"],
  [10, 21, "emergency-rescue"],
  [25, 22, "wormholes"],
  [26, 23, "astar-grid"],
  [27, 24, "heuristic-validation"],
  [28, 25, "eight-puzzle"],
  [6, 26, "bfs-bipartite"],
  [29, 27, "bipartite-matching"],
  [30, 28, "pilot-pairing"],
  [31, 29, "edmonds-karp"],
  [32, 30, "min-cost-max-flow"],
] as const;
...
export function ch07IdAliases(
  ...
  const expected = CH07_RENUMBERING.map(([, next, slug]) => ({
    id: `07E${pad(next)}`,
    name: `E-07-${pad(next)}-${slug}`,
  }));
  if (!expected.every((entry) => chapter.some((lab) => lab.id === entry.id && lab.name === entry.name))) return [];
  return [
