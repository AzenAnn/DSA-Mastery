import type { CaseResult, ProjectCurrentState, ProjectScoreResult, ScoreResult, Verdict } from "../cli/client";
import type { LabEntry, ProjectLab, ProgramLab } from "../labs/discovery";
import type { ProjectProgress } from "./project";
import type { ActivityEvent } from "./stats";
import type { QuizQuestion } from "@dsa/lab-core";
import { copyFile, mkdir, rm } from "node:fs/promises";
import path from "node:path";
import * as vscode from "vscode";
import { studentSourcePath } from "../labs/discovery";
import { remapEventKeys, remapRecordKeys } from "./keys";
import { mergeLabProgress, mergeQuizProgress } from "./merge";
import { CH04_MIGRATION, ch04IdAliases } from "./migrations/ch04";
import { CH07_MIGRATION, ch07IdAliases } from "./migrations/ch07";
import { projectProgressPassed, summarizeProjectSubmission } from "./project";
...
export class ProgressTracker {
  ...
  private loadProjects(): ProjectStore {
    ...
    if (raw === undefined || raw.schemaVersion !== 1 || typeof raw.projects !== "object") {
  ...
  getProject(labId: string): ProjectProgress | undefined {
    ...
    if (current === undefined) return stored === undefined ? undefined : { ...stored, currentUnknown: true };
    return {
      ...stored,
      submissionCount: stored?.submissionCount ?? 0,
      automatedScore: current.automatedScore,
      automatedMax: current.automatedMax,
      manualPending: current.manualPending,
      provisionalTotal: current.provisionalTotal,
      total: current.total,
      automatedFull: current.automatedFull,
      internalError: current.internalError,
      current,
  ...
  private async pruneHistory(progress: LabProgress): Promise<void> {
    ...
    await Promise.all(
      dropped.map((entry) =>
        rm(path.dirname(path.join(this.context.globalStorageUri.fsPath, entry.snapshot)), {
          recursive: true,
          force: true,
        }),
      ),
