import type { ProjectCurrentState, ProjectScoreResult, ProjectStatus, ProjectTaskResult, Verdict } from "../cli/client";

interface ProjectCaseSummary {
  id: string;
  verdict: Verdict;
  points: number;
  maxPoints: number;
...
export function projectProgressPassed(
  ...
  return (
    !progress.currentUnknown &&
    (progress.current
      ? progress.current.complete
      : progress.automatedFull && progress.manualPending === 0 && !progress.internalError)
  );
}
  ...
  return {
    ...
    diagnostic:
      task.build && !task.build.ok
        ? `${task.build.build?.stdout ?? task.build.configure?.stdout ?? ""}\n${task.build.build?.stderr ?? task.build.configure?.stderr ?? ""}`.slice(
            0,
            16000,
          )
        : undefined,
