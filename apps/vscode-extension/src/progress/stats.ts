 ...
 */
export function buildHeatmap(events: readonly ActivityEvent[], kind: "submit" | "pass", from: Date, to: Date): Heatmap {
  ...
  // 逐日重建本地日期而不是原地 setDate，跨夏令时也不会漂移。
  for (
...
export function buildChapterBars(
  chapters: readonly {
    chapter: number;
    chapterTitle: string;
    labs: readonly { id?: string; name: string; type: "program" | "quiz" | "project" }[];
  }[],
