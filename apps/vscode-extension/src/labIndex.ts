...
import { parseQuizQuestions } from './quiz';
  ...
  for (const candidate of manifest.tasks) {
    ...
    if (kind === "stdio") {
      ...
      for (const key of ["buildTargets", "moduleTargets"] as const) {
        ...
        if (names === undefined || names.length === 0 || new Set(names).size !== names.length || names.some((name) => !/^\w[\w.+-]*$/iu.test(name))) return undefined;
...
function buildBase(
  ...
  const legacyNames = slug !== undefined && Number.isSafeInteger(order) && order !== Number.MAX_SAFE_INTEGER
...
async function loadTestCasesAt(root: string, casesFile: string): Promise<LoadedTestCase[]> {
  ...
  for (const testCase of cases) {
    if (testCase === null || typeof testCase.id !== "string" || typeof testCase.input !== "string" || typeof testCase.expected !== "string") continue;
