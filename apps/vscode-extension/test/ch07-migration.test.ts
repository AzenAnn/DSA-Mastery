...
it("all 32 original identities survive without mixing retired or overlapping scores", () => {
  ...
  expect(ch07IdAliases([...labs, { id: "07E02", name: "E-07-02-connected-components", type: "program" }], [])).toStrictEqual([]);
...
it("real Ch7 discovery has thirty matching identities and legacy directory aliases", async () => {
  ...
  try {
    ...
    await build({ entryPoints: [path.join(extensionRoot, "src/labIndex.ts")], outfile: bundle, bundle: true, platform: "node", format: "cjs" });
    ({ discoverProgramLabs } = createRequire(import.meta.url)(bundle) as typeof import("../src/labIndex.ts"));
  } finally {
...
it("tracker backs up, migrates both chapters once, preserves snapshots and survives restart/reset", async () => {
  ...
  try {
    ...
    const ch4 = CH04_RENUMBERING.filter(([, next]) => next <= 31).map(([, next, slug]) => ({
      id: `04E${pad(next)}`, name: `E-04-${pad(next)}-${slug}`, type: "program", legacyNames: [] as string[],
    }));
    ...
    expect(tracker.get("07E02").history.map((item: { snapshot: string }) => item.snapshot).sort()).toStrictEqual(["submissions/iterative-dir/main.cpp", "submissions/iterative-id/main.cpp"]);
