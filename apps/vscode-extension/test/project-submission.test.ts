  ...
  try {
    ...
    const fake = {
      ...
      cli: {
        ...
        scoreProject: async () => { calls += 1; await new Promise<void>((resolve) => { release = resolve; scoring(); }); return { ...current, current }; },
      },
    };
    (globalThis as unknown as { projectFixture: unknown }).projectFixture = fake;
    const output = path.join(root, "panel.cjs");
    await build({ entryPoints: [path.join(extensionRoot, "src/panel.ts")], outfile: output, bundle: true, platform: "node", format: "cjs", plugins: [{ name: "project-fixture", setup(builder) {
