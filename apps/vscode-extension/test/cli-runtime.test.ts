...
it("CLI checks trust and explicit Node paths and falls back when PATH has no Node", async () => {
  ...
  try {
    ...
    await build({
      entryPoints: [path.join(extensionRoot, "src/cli/client.ts")],
      outfile: output,
      bundle: true,
      platform: "node",
      format: "cjs",
      ...
      plugins: [
        {
          ...
          setup(builder) {
            ...
            builder.onLoad({ filter: /.*/, namespace: "fixture" }, () => ({
              contents: "module.exports = globalThis.runtimeFixture;",
            }));
          },
        },
      ],
    });
    await mkdir(path.join(root, "packages/lab-cli/dist"), { recursive: true });
    await writeFile(
      path.join(root, "packages/lab-cli/dist/cli.js"),
      'console.log(JSON.stringify({reportVersion:1,command:"project-status",ok:true,result:{tasks:[],complete:false}}));',
    );
    const { readProjectCurrent } = createRequire(import.meta.url)(output) as typeof import("../src/cli/client.ts");
    fake.workspace.isTrusted = false;
    await expect(readProjectCurrent(root, ".")).rejects.toThrow(
      expect.objectContaining({ code: "WORKSPACE_UNTRUSTED" }),
    );
