...
it("CLI checks trust and explicit Node paths and falls back when PATH has no Node", async () => {
  ...
  try {
    ...
    await build({ entryPoints: [path.join(extensionRoot, "src/cli.ts")], outfile: output, bundle: true, platform: "node", format: "cjs",
    ...
    await writeFile(path.join(root, "packages/lab-cli/dist/cli.js"), 'console.log(JSON.stringify({reportVersion:1,command:"project-status",ok:true,result:{tasks:[],complete:false}}));');
    const { readProjectCurrent } = createRequire(import.meta.url)(output) as typeof import("../src/cli.ts");
