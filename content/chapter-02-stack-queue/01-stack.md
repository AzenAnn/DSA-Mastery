...
本文自己实现的示例选择在空栈时抛异常；不同 API 的失败契约可能不同。例如使用 std::stack 时，调用 top() 或 pop() 前应先保证栈非空。
...
这段代码里我们使用了 C++ 标准库中的 `std::stack`，std::stack 默认使用 std::deque 作为底层容器。deque 通常采用分段连续存储，与本节介绍的单一连续数组和逐结点链表都不完全相同。
</details>
