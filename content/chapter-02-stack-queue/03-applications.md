...
中缀表达式 `a + b * c` 依赖括号与优先级，而后缀表达式（Reverse Polish Notation，RPN，逆波兰表达式）没有括号、从左到右即可求值。后缀求值和中缀转后缀都使用栈，但栈中保存的对象不同：前者保存操作数，后者保存尚未输出的运算符。

#### 例题：逆波兰表达式求值

> 题目来源：改编自 [LeetCode 150：逆波兰表达式求值](https://leetcode.cn/problems/evaluate-reverse-polish-notation/)。本例使用课程自定义输入输出协议。

给定 `n` 个由空白分隔的 token。token 要么是十进制整数，要么是二元运算符 `+`、`-`、`*`、`/`。从左到右扫描：整数直接入栈；遇到运算符时，先弹出右操作数 `right`，再弹出左操作数 `left`，计算 `left op right` 后把结果压回。

合法表达式最终必须恰好留下一个结果。操作数不足、除零、未知 token 或最终栈基数不为 1 时统一输出 `ERROR`。除法采用 C++ 整数除法的向零截断语义；题目保证合法计算的中间结果处于 64 位有符号整数范围内。
...
| `2` | `[2]` | 整数直接入栈 |
| `1` | `[2, 1]` | 整数直接入栈 |
| `+` | `[3]` | 先弹出 `1`，再弹出 `2`，计算 `2 + 1` |
| `3` | `[3, 3]` | 整数直接入栈 |
| `*` | `[9]` | 计算 `3 * 3` |

扫描结束时栈中恰好剩一个值，因此结果为 `9`。
:::

```cpp:line-numbers [rpn-evaluation-example.cpp]
#include <cstddef>
#include <iostream>
#include <stdexcept>
#include <string>
...
::: pitfall 易错点 · 局部可计算不等于整式合法
- `8 3 -` 必须计算 `8 - 3`，不能交换为 `3 - 8`；
- `-3` 是一个完整的整数 token，不是减号运算符；
- `1 +` 在运算符处缺少操作数，`4 0 /` 会除零，`1 2` 则在结束时残留两个值；三者都应输出 `ERROR`；
- 一旦表达式已经判定非法，仍需读完声明的 `n` 个 token，避免输入协议与处理状态脱节。
:::

#### 中缀转后缀：调度场算法

使用**调度场算法**（shunting-yard，由 Edsger Dijkstra 提出）可以把中缀表达式转换为后缀表达式：操作数直接输出，运算符与栈顶比较优先级。为了聚焦栈操作，下面约定每个操作数都是单个字母或数字，只支持二元 `+`、`-`、`*`、`/`；相邻操作数、一元运算符和空括号都属于非法输入。定义优先级 `P`：
...
::: warning 示例边界
逆波兰例题支持由空白分隔的多位整数和负数，但不检查合法运算过程中的 64 位溢出。调度场示例为了突出运算符栈，只接受单字符操作数以及左结合的二元 `+`、`-`、`*`、`/`。如果要把两段代码直接组合为通用计算器，还需要统一 token 格式，并补充一元运算符与溢出检查。
:::
...
::: example 示例 · 完整转换并求值
对当前“单字符 token”版本，中缀表达式 `(2 + 3) * (7 - 4)` 的转换结果是紧凑字符串 `23+74-*`。若把结果交给前面的“空白分隔 token”求值器，应先表示为 `2 3 + 7 4 - *`：`2 3 +` 得到 `5`，`7 4 -` 得到 `3`，最后计算 `5 3 *`，结果为 `15`。
:::
...
两者都可以用栈建模。任务说明见 [Lab 02-P-01：可撤销浏览器——栈的综合应用](../../labs/chapter-02/project/P-02-01-undoable-browser/README.md)。
...
任务说明见 [Lab 02-P-02：超市收银模拟——队列的综合应用](../../labs/chapter-02/project/P-02-02-supermarket-checkout/README.md)。

### 栈 + 队列联动：停车场管理

停车场内部车道用**栈**（后进的车堵住先进的车，取车要倒出来），门外便道用**队列**（先到先进场）。这是“最近优先”与“先到优先”在同一业务流程里协作的典型例子。任务说明见 [Lab 02-P-03：停车场管理——栈与队列的综合应用](../../labs/chapter-02/project/P-02-03-parking-lot-management/README.md)。
...
4. 浏览器访问新页面时为什么要清空前进栈？不清空会发生什么？（可对照 Lab 02-P-01）
5. 一个系统需要“后到的请求先处理”（如撤销栈），它该用栈还是队列？如果改成“等待最久的请求先处理”呢？

::: details 查看参考思路
1. 后缀表达式为 `2 3 + 7 4 - *`：先计算 `2 3 +` 得到 `5`，再计算 `7 4 -` 得到 `3`，最后计算 `5 3 *` 得到 `15`。用空格分隔 token，可以避免多位数操作数产生歧义。
