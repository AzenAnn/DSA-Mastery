...
::: counterexample 反例 · 延迟标记造成重复入队
设有向边为 $0\to1$、$0\to2$、$1\to3$、$2\to3$。若只在顶点出队时标记，处理顶点 $1$ 时会把 $3$ 入队；随后处理 $2$ 时，$3$ 尚未出队，仍显示“未访问”，于是被再次入队。

在首次发现、入队或入栈前立即标记，才能建立“每个顶点最多进入待处理容器一次”的不变量。出队后再跳过重复项虽然可能保持结果正确，却增加了无意义的队列空间和处理成本。
:::
...
::: proof 单源遍历为什么恰好得到可达集合
**不会多收：** 起点 $s$ 通过长度为 $0$ 的路径可达自身；此后每个新顶点 $v$ 都由某个已发现顶点 $u$ 沿边 $u\to v$ 发现，把这条边接在 $s\leadsto u$ 后面即可得到 $s\leadsto v$。

**不会漏收：** 假设某个从 $s$ 可达的顶点最终未被发现。在一条从 $s$ 到该顶点的路径上，取第一个未被发现的顶点 $v$；它的前驱 $u$ 已被发现并最终会被处理。算法扫描 $u$ 的邻接记录时必然看到 $u\to v$ 并发现 $v$，产生矛盾。

因此栈与队列只改变待处理顶点的顺序，不改变最终集合。
:::
...
若顶点集非空，返回值为 `1` 当且仅当无向图连通。没有边的孤立顶点仍会触发一次搜索，所以它单独构成一个连通分量。空图会返回 `0`；不同教材对空图是否称为连通图可能采用不同约定，程序接口应显式说明，而不能把 `count == 1` 无条件套到 $n=0$。
...
$$
0\to1,\ 1\to0,\ 1\to2,\ 2\to3,\ 3\to2,\ 3\to4.
$$

三个 SCC 分别是 $A=\{0,1\}$、$B=\{2,3\}$、$C=\{4\}$，凝聚图为 $A\to B\to C$。若第一轮从 $0$ 开始并按编号访问邻居，发现顺序为 $0,1,2,3,4$，完成顺序为 $4,3,2,1,0$，因此第二轮按 `0,1,2,3,4` 扫描转置图：

| 第二轮起点 | 在转置图中收集的顶点 | SCC |
| ---: | --- | --- |
| $0$ | $0,1$ | $A$ |
| $2$ | $2,3$ | $B$ |
| $4$ | $4$ | $C$ |

处理 $A$ 时，转置图中的跨分量边方向为 $C\to B\to A$，所以搜索无法从 $A$ 跑到尚未处理的 $B$ 或 $C$；随后同理。若错误地按完成时间递增、先从 $4$ 开始，转置搜索会沿 $C\to B\to A$ 一次吞掉三个分量。
...
::: proof
只需比较第一轮最先发现两个分量中任意一个顶点的时刻。

- 若先发现 $C$，DFS 可以沿凝聚边进入 $D$；由于不同 SCC 不可能从 $D$ 返回 $C$，$D$ 会在 $C$ 之前完成。
- 若先发现 $D$，在发现 $C$ 前以 $D$ 为根的搜索必须已经完成；否则从 $D$ 的搜索能到达后来发现的 $C$，再结合 $C\to D$ 就会说明二者互相可达、应属于同一 SCC。因此之后发现的 $C$ 仍具有更大的完成时间。

两种情况都得到 $f(C)>f(D)$。
:::

在尚未标号分量组成的凝聚子图中，最大完成时间所属分量没有来自其他未处理分量的入边；否则引理会让那个前驱拥有更大的完成时间。转置后这些边全部反向，所以该分量没有通往其他未处理分量的出边。第二轮 DFS 不会越界；而分量内部在转置后仍强连通，所以它又不会漏掉本分量顶点。删除已标号分量并重复此论证，即可得到全部 SCC。
...
有向图包含 $0\to1$、$1\to0$ 与 $1\to2$。第一轮从 $0$ 开始 DFS，完成顺序为 $2,1,0$。第二轮若错误地按完成时间递增、先从 $2$ 在转置图上搜索，会发生什么？正确顺序会得到哪些 SCC？

::: details 点击展开答案
转置图包含 $1\to0$、$0\to1$ 与 $2\to1$。若先从 $2$ 开始，可以沿 $2\to1\to0$ 访问三个顶点，错误地把两个 SCC 合并。

正确做法按完成时间递减处理 `0,1,2`：先从 $0$ 收集 $\{0,1\}$；顶点 $1$ 已标号，随后从 $2$ 收集 $\{2\}$，得到两个 SCC。
:::
...
- [Lab 07-E-02：显式栈 DFS](../../labs/chapter-07/exercise/E-07-02-iterative-dfs/README.md)：处理大规模深图并核对访问顺序；
