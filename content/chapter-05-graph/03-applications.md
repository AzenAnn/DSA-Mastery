...
把所有边按权值排序，从小到大逐条考虑：若边的两个端点尚不在同一连通分量，就加入并合并（用并查集维护连通性）。排序 `O(m \log m)`，并查集操作近似 `O(1)`，总复杂度 `O(m \log m)`——稀疏图明显优于 Prim 的 `O(n²)`。
