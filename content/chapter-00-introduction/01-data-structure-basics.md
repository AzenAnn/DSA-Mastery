...
假设你要编写一个学生信息管理程序。每名学生都有学号、姓名和成绩，程序需要完成以下工作：
...
**数据**是对客观事物的描述，是能够被计算机输入、存储和处理的符号。例如，`20260017`、`"李明"` 和 `86.5` 都可以成为程序中的数据。
...
**数据对象**是具有相同性质的数据元素的集合。例如，本班所有学生记录共同组成一个学生数据对象。
...
集合结构中的元素除了“同属于一个集合”之外，没有额外的先后或层级关系。例如，一组互不重复的课程编号可以看作一个集合。后续学习散列等结构时，会再次遇到强调成员关系与快速判定的场景。

### 线性结构

线性结构中的元素形成一对一的先后关系。除第一个元素外，每个元素最多有一个直接前驱；除最后一个元素外，每个元素最多有一个直接后继。按录入顺序排列的学生记录、播放列表和排队队伍都是线性结构。第 1 章将从线性表开始研究这种关系。

### 树形结构

树形结构表达一对多的层级关系。一个元素可以有多个直接后继，但通常只有一个直接前驱。学校的组织结构、文件目录和表达式语法树都是树形关系。后续树章节会研究节点之间的父子关系及其遍历方法。

### 图结构

图结构表达多对多的任意关联。一个元素可以与多个其他元素直接相连，例如社交关系、道路网络和课程先修关系。后续图章节会研究顶点、边、路径以及不同的图存储方式。

## 常见存储结构

确定逻辑关系后，还要选择怎样把元素和关系放进内存。下面仍使用同一组学生记录进行比较。
...
学校需要维护一个小型通讯录。联系人按用户设定的顺序显示，程序需要顺序浏览全部联系人、按学号查找联系人、在任意位置插入新联系人，并删除离校学生。通讯录初期只有几十条记录，之后可能持续增长。
