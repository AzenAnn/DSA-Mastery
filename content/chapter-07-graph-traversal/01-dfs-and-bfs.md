...
遍历的代价由"找邻居"的代价决定。第 6 章的[邻接矩阵](../chapter-06-graph-foundations/01-adjacency-matrix.md)能 $O(1)$ 判断两点是否相邻，但扫一遍某个顶点的全部邻居要 $O(n)$；大多数图算法也采用第 6 章介绍过的**邻接表**。下面的代码把它建出来，并让每个顶点的邻居按升序排列：
...
一棵树的 DFS 就是[第 4 章二叉树](../chapter-04-tree/01-binary-tree.md)的**先序遍历**：访问根，再依次先序遍历每个子树。图的 DFS 只是多了一道约束——图里有环，要用 `visited` 挡住"走回祖先"的回头路。图的递归代码和树的递归代码几乎一样，只是把 `left/right` 换成了"邻居列表"。
:::
...
以下代码假设 `graph[u]` 已经按升序排序（见"用什么存图"小节）。逆序入栈是为了利用栈的后进先出特性补偿入栈顺序，使弹出顺序与升序一致。
...
这份代码用栈 `pending` 存放"已经被决定要去、但还没访问"的顶点：每轮弹出栈顶访问，再把这个顶点的未访问邻居逆序压栈。逆序压栈是为了让最小编号的邻居成为栈顶，弹出时正好是升序；入栈时立刻标记，则同一个顶点绝不会被压第二次。栈空时，从起点能到达的顶点就全部访问完了。

::: pitfall 易错点 · 两种 DFS 的访问顺序不必相同
- **区别**：递归版在进入时标记、立即深入；显式栈版在入栈时标记、出栈后才访问。
- **后果**：入栈顺序、标记时机不同，访问顺序可能不同；始终一致的是"访问过的顶点集合"。
- **约定**：想和递归版顺序一致，就用"逆序入栈 + 入栈时标记"，上面的代码正是如此。
:::
...
本节讨论的是**无权图**，即每条边的代价都视为 1；BFS 的"按层"只在这一前提下成立，带权图要改用[7.2 节](./02-minimum-spanning-tree-and-shortest-path.md)的 Dijkstra（一种求带权最短路径的经典算法）。BFS 的规则一句话：**先访问起点，再访问起点的全部邻居，再访问邻居的邻居**。前面小图里，`{1, 2}` 是第 1 层，`{3, 4}` 是第 2 层，BFS 严格按层访问。队列的"先进先出（FIFO）"恰好保证这一点：先入队的（更近的）一定先出队。第 2 章的[队列应用](../chapter-02-stack-queue/03-applications.md)已预告过这种"按层扩散"，这里正式用在图上。
