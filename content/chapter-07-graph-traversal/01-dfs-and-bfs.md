...
图没有固定的“第一个元素”或“下一个元素”。要系统地访问顶点，必须规定选择起点、邻居顺序以及待访问顶点的调度方式，这就是**图遍历**（graph traversal）。本节介绍深度优先搜索（DFS）和广度优先搜索（BFS）：DFS 沿当前分支深入，无法继续时回退；BFS 从起点按距离层次向外扩展。二者都能覆盖起点可达的顶点，但访问顺序与适用任务不同。

[第 6.3 节](../chapter-06-graph-foundations/03-graph-traversal-connectivity.md)已经用遍历计算可达集合、无向连通分量、弱连通分量和强连通分量。本节保留一份简洁的 DFS/BFS 骨架以便独立阅读，但不再展开连通分量算法；重点转向**访问顺序为什么不同，以及这些顺序能证明和解决什么**。四个问题贯穿全节：
...
::: definition 定义 · 图遍历
设 $G=(V,E)$ 是无向图，$s\in V$ 是起点。从 $s$ 出发的**图遍历**按既定规则沿边访问 $s$ 所在连通分量的全部顶点，并让每个顶点只被正式处理一次。图不连通时，单次遍历只覆盖起点所在的分量；若要覆盖全图，还需从其他未访问顶点重新启动搜索。
:::

::: tip 有向图怎么办
有向图的情形类似，但沿边移动必须遵守方向：邻居列表改为出边列表，单次遍历覆盖的是**从起点可达的顶点集合**，不要把它误称为强连通分量。本节示例与复杂度分析主要使用无向图；后文讨论 DFS 边分类和有向环检测时会明确加入方向条件。
:::
...
递归的调用栈由系统保存“返回后从哪里继续”等信息。如果图是一条很长的链，递归深度会达到顶点数 $n$，可能耗尽调用栈。显式栈把待处理状态存放在程序管理的容器中；它的最坏元素数仍为 $O(n)$，但通常不受较小的调用栈容量限制，而且可以只保存算法需要的字段。若可用内存不足，显式栈同样会失败。
...
DFS 用栈式调度（后进先出，先深入），BFS 用队列调度（先进先出，先扩散），这是两者访问次序的核心区别。下面代码同时维护访问顺序 `order` 和距离 `dist`：`dist[v]` 表示 $s$ 到 $v$ 的最少边数，用 `-1` 兼作"未访问"标记。
...
3. 若图改用邻接矩阵存储，BFS 的复杂度变成多少？在顶点数很大的稀疏图上，为什么通常应优先考虑邻接表？
4. 图不连通时，从固定源点 $s$ 运行一次 BFS 后，不可达顶点的距离应如何表示？如果目标改为遍历全图并为每个连通分量建立一棵 BFS 树，又该怎么做？
