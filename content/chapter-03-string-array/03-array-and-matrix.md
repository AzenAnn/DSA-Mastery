...
1. **先确认下标约定**：本章实验统一使用 0-based 下标；若题目改成 1-based，必须先把每一维下标减一，再代入公式。
2. **再确认存储顺序**：行优先时，某一维的步长由它后面各维长度的乘积决定；列优先则相反。不要只记二维公式。
3. **最后确认结构范围**：三对角矩阵只存 `|i-j|≤1` 的位置；对称矩阵先把 `(i,j)` 规范到存储三角区；稀疏矩阵则要区分“没有三元组”和“值为零”。

常见错误包括：

- 把三对角矩阵的 `k = 2i + j` 用在对角线以外的位置；
- 计算多维数组步长时漏乘最后一维，或把维数 `n` 与某一维长度混淆；
- 稀疏矩阵转置后只交换行列，却忘记按行优先重新排序；
- 用 `int` 保存大规模数组总元素数或地址偏移，导致乘法溢出。
...
冷知识，如果不会配置环境又不会赛博出国的话，可以下载Trae或者WorkBuddy之类的国产Agent并接deepseek或使用免费额度进行自动配置。亦或是点开以下链接进入作者主页获取一键配置脚本：[作者主页](https://www.bilibili.com/video/BV1GJ411x7h7/?spm_id_from=333.337.search-card.all.click&vd_source=94105e6e7f45c66d2d663b09af6ae810)
