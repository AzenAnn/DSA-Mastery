...
业务约束如何实现？在 `SubmitCommand` 里保存一个指向"上一条命令类型"的检查函数（或由 `Browser::doCommand` 预检：读取 `undo_stack_.top()` 是否 `InputCommand`）。不满足时抛异常，由 `Browser` 接住并打印日志，程序不崩溃。

### 模块四：结构化日志（必做）

每次状态变化都打印一行日志，能据此重建浏览器完整状态：
...
建议封装一个 `Logger` 类，统一输出时间戳、动作、当前页与四个栈的实时快照。
