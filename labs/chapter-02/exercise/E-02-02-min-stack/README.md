...
1. **重复最小值会让恢复时机变得不明确。** 例如依次压入 `3、2、2`。如果辅助栈只在最小值严格变小时记录 `[3, 2]`，弹出一个 `2` 后若同时删除辅助栈顶，就会错误地把最小值恢复为 `3`，但主栈中其实还剩一个 `2`。解决方法是为相同最小值也保存一份记录，或者为每个最小值额外记录出现次数。
2. **同步保存更简单，稀疏保存可能更省空间。** 同步方案让主栈和最小值栈始终等长，压栈、弹栈可以机械地同步执行，重复最小值也自然正确；代价是每个元素都对应一份最小值记录。只在最小值变化时保存，可以在最小值很少变化时减少记录，但必须在弹栈时比较数值并正确维护重复次数，实现和不变量都更复杂。两种方案最坏空间复杂度仍是 `O(n)`。
