...
使用输出字符串保存已经确定顺序的操作数和运算符，使用运算符栈暂存尚不能输出的运算符。扫描时维护 `expectOperand`：为真时只能读操作数或左括号，为假时只能读运算符或右括号。栈中除左括号外始终按待处理优先级排列。

### 算法步骤

1. 操作数直接追加到输出；左括号入栈；
2. 读到运算符时，弹出栈顶所有优先级不低于当前运算符的同层运算符，再压入当前运算符；
3. 读到右括号时弹栈至对应左括号，找不到左括号则非法；
4. 扫描结束后若仍在等待操作数或栈中有未关闭左括号，输出 `ERROR`，否则弹出剩余运算符。

### 正确性说明

优先级不低于当前运算符的栈顶运算符必须先计算，左结合性因此得到保证；左括号隔离了括号内外的运算符。每个操作数按原顺序输出，每个运算符在所有更高或同级的左侧运算符处理后输出，所以最终字符串满足后缀表达式的计算顺序。

### 复杂度分析

- 时间复杂度：`O(|s|)`，每个字符及运算符最多入栈、出栈一次；
- 额外空间：`O(|s|)`，包括输出字符串和运算符栈。

### 边界注意

- `a(b)`、`ab`、`a++b` 等违反操作数/运算符交替规则的表达式必须拒绝；
- 右括号不能出现在等待操作数的状态，空括号 `()` 也因此非法；
- 扫描结束时要继续检查栈内是否残留左括号。
