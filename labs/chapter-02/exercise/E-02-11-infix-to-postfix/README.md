...
description: "练习中缀表达式转后缀，强化栈与队列的不变量、边界和复杂度。"
order: 18
chapter: 2
labId: "02E11"
chapterTitle: "栈与队列"
...
掌握中缀表达式转后缀的栈/队列建模，明确空、满、重复、循环或不可达边界，并能说明时间与空间复杂度。
...
请完成“中缀表达式转后缀”。一行是空格分隔的非负整数、四则运算符和括号组成的合法中缀式；输出空格分隔的后缀式。 输出只保留题目要求的结果，调试信息写入标准错误。
...
```powershell
pnpm lab:doctor -- labs/chapter-02/exercise/E-02-11-infix-to-postfix
