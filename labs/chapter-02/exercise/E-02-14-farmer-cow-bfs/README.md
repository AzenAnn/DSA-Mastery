...
把每个合法位置看成图的一个顶点，`x-1`、`x+1` 和 `2*x` 是三条单位代价边。使用 BFS 队列按距离分层扩展；`distance[x]` 记录从起点到 `x` 的最短步数，非 `-1` 表示该位置已经入队。

### 算法步骤

1. 将 `start` 的距离设为 0 并入队；
2. 每次取出一个位置，枚举三个转移；
3. 忽略越界位置和已经访问的位置，对新位置设置 `distance[current] + 1` 后入队；
4. 目标位置出队后即可结束，最终输出其距离。

### 正确性说明

BFS 先处理距离为 0 的顶点，再处理距离为 1、2……的顶点。由于每条边代价相同，某个位置第一次入队时使用的路径已经是最短路径；访问标记在入队时设置也保证它不会被重复加入。因而输出的 `distance[target]` 是最少步数。

### 复杂度分析

- 时间复杂度：`O(limit)`，每个位置最多出队一次并检查三个转移；
- 额外空间：`O(limit)`，用于距离数组和队列。

### 边界注意

- `start == target` 直接得到 0；
- `2*x`、`x-1`、`x+1` 都要先检查是否落在 `0..limit-1`；
- 不能因为某次向后移动看似变远就删除它，它可能是最短路径的一部分。
