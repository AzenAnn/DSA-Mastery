---
title: "Lab 02-E-20：网格多源扩散"
description: "练习网格多源扩散，强化栈与队列的不变量、边界和复杂度。"
order: 27
chapter: 2
labId: "02E20"
chapterTitle: "栈与队列"
...
掌握网格多源扩散的栈/队列建模，明确空、满、重复、循环或不可达边界，并能说明时间与空间复杂度。
...
第一行给出正整数 `h w`，随后 `h` 行，每行恰好 `w` 个字符。`S` 为源，`T` 为目标，`.` 为普通空地，`#` 为不可通行障碍。所有源在时刻 0 同时开始，每步向上、下、左、右移动一格，源和目标也可通行。输出一行，按从上到下、同一行从左到右的顺序列出每个 `T` 到最近源的最短步数；不可达（包括没有源）输出 `-1`。没有目标时输出单个 `0`。保证 `h*w≤200000`，时间与空间要求均为 `O(h*w)`。
...
```powershell
pnpm lab:doctor -- labs/chapter-02/exercise/E-02-20-multi-source-grid-bfs
pnpm lab:run -- labs/chapter-02/exercise/E-02-20-multi-source-grid-bfs
pnpm lab:score -- labs/chapter-02/exercise/E-02-20-multi-source-grid-bfs
```
