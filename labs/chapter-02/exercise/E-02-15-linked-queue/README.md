---
title: "Lab 02-E-15：链式队列"
description: "练习链式队列，强化栈与队列的不变量、边界和复杂度。"
order: 22
chapter: 2
labId: "02E15"
chapterTitle: "栈与队列"
...
掌握链式队列的栈/队列建模，明确空、满、重复、循环或不可达边界，并能说明时间与空间复杂度。
...
第一行给出操作数 `q`，随后恰好 `q` 行命令。`ENQ x` 将整数 `x` 放入队尾并输出 `OK`；`DEQ` 删除并输出队头；`FRONT` / `REAR` 只读取并输出队头/队尾。空队列的删除或读取输出 `EMPTY`，状态不变。每条命令输出一行。要求使用链结点保存元素，删除最后一个结点后正确复位首尾指针，程序结束时释放剩余结点。
...
```text
8
DEQ ENQ 0 FRONT REAR DEQ DEQ ENQ 10 REAR
```
...
```powershell
pnpm lab:doctor -- labs/chapter-02/exercise/E-02-15-linked-queue
pnpm lab:run -- labs/chapter-02/exercise/E-02-15-linked-queue
pnpm lab:score -- labs/chapter-02/exercise/E-02-15-linked-queue
```
