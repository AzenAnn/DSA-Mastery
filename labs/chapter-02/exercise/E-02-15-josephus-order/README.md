...
队列从队头开始表示当前仍在圈中的人，队头就是本轮报 1 的人。每轮把前 `k-1` 人移到队尾，随后删除队头；删除后新的队头自然是下一轮报 1 的人。
...
轮转后队头正好是从当前报数起第 `k` 个人，因此删除它符合规则；被删除者之后的存活者成为新的队头，满足下一轮从其后重新报 1。每轮都保持队列按圈中顺序排列，所以输出完整且顺序正确。

### 复杂度分析

- 时间复杂度：`O(nk)`，每轮最多移动 `k-1` 人；
- 额外空间：`O(n)`，队列保存尚未出列的人。

### 边界注意

- `k=1` 时不需要轮转，直接依次出队；
- `k` 可能大于当前队列长度，仍按题目定义执行 `k-1` 次轮转；
- 输出的是完整出列顺序，不是只保留最后一个人的经典 Josephus 结果。
