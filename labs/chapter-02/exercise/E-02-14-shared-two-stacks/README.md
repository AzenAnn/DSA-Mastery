---
title: "Lab 02-E-14：共享数组双栈"
description: "练习共享数组双栈，强化栈与队列的不变量、边界和复杂度。"
order: 21
chapter: 2
labId: "02E14"
chapterTitle: "栈与队列"
...
掌握共享数组双栈的栈/队列建模，明确空、满、重复、循环或不可达边界，并能说明时间与空间复杂度。
...
第一行给出总容量 `C` 和操作数 `q`，随后恰好 `q` 行。`LPUSH x` / `RPUSH x` 向左栈/右栈压入整数，成功输出 `OK`；两栈元素总数等于 `C` 时输出 `FULL` 且不改变状态。`LPOP` / `RPOP` 删除并输出指定栈的栈顶，空栈输出 `EMPTY` 且不改变状态。两栈分别后进先出，可按任意比例共享全部 `C` 个槽位。每条命令输出一行。
...
```text
3 8
LPUSH 0 RPUSH 10 LPUSH 1 RPUSH 11 LPOP 0 RPOP 0 RPOP 0 LPOP 0
```
...
```powershell
pnpm lab:doctor -- labs/chapter-02/exercise/E-02-14-shared-two-stacks
pnpm lab:run -- labs/chapter-02/exercise/E-02-14-shared-two-stacks
pnpm lab:score -- labs/chapter-02/exercise/E-02-14-shared-two-stacks
```
