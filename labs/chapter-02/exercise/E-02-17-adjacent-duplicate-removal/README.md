---
title: "Lab 02-E-17：相邻重复字符消除"
description: "练习相邻重复字符消除，强化栈与队列的不变量、边界和复杂度。"
order: 24
chapter: 2
labId: "02E17"
chapterTitle: "栈与队列"
...
# Lab 02-E-17：相邻重复字符消除

## 学习目标

掌握相邻重复字符消除的栈/队列建模，明确空、满、重复、循环或不可达边界，并能说明时间与空间复杂度。
...
第一行给出长度 `n`，第二行是由小写英文字母和数字组成的字符串；`n=0` 时第二行为空。从左到右处理字符：若当前字符与已保留前缀的最后一个字符相同，就删除这一对，否则保留当前字符。删除后相邻的新字符也继续按该规则处理。输出最终字符串；结果为空时输出 `EMPTY`。
...
```powershell
pnpm lab:doctor -- labs/chapter-02/exercise/E-02-17-adjacent-duplicate-removal
pnpm lab:run -- labs/chapter-02/exercise/E-02-17-adjacent-duplicate-removal
pnpm lab:score -- labs/chapter-02/exercise/E-02-17-adjacent-duplicate-removal
```
