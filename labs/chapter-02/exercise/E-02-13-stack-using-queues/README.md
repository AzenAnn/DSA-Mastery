---
title: "Lab 02-E-13：用队列实现栈"
description: "练习用队列实现栈，强化栈与队列的不变量、边界和复杂度。"
order: 20
chapter: 2
labId: "02E13"
chapterTitle: "栈与队列"
...
掌握用队列实现栈的栈/队列建模，明确空、满、重复、循环或不可达边界，并能说明时间与空间复杂度。
...
第一行给出操作数 `q`，随后恰好 `q` 行，每行是一条命令：`push x` 将整数 `x` 压入栈且不输出；`pop` 删除并输出栈顶；`top` 只输出栈顶。空栈执行 `pop` 或 `top` 时输出 `EMPTY`，状态不变。结果按命令顺序逐行输出。要求使用队列的入队、队头读取和出队操作实现后进先出语义。
...
```powershell
pnpm lab:doctor -- labs/chapter-02/exercise/E-02-13-stack-using-queues
pnpm lab:run -- labs/chapter-02/exercise/E-02-13-stack-using-queues
pnpm lab:score -- labs/chapter-02/exercise/E-02-13-stack-using-queues
```
