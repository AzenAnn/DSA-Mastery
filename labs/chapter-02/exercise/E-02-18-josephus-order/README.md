---
title: "Lab 02-E-18：约瑟夫出队顺序"
description: "练习约瑟夫出队顺序，强化栈与队列的不变量、边界和复杂度。"
order: 25
chapter: 2
labId: "02E18"
chapterTitle: "栈与队列"
...
掌握约瑟夫出队顺序的栈/队列建模，明确空、满、重复、循环或不可达边界，并能说明时间与空间复杂度。
...
一行给出人数 `n` 和正整数步长 `m`。编号 `1..n` 的人按编号围成环，第一次由编号 1 报 1，每次报到 `m` 的人出队；下一位仍在环中的人重新从 1 报数，直到全部出队。输出一行完整出队序列，编号以空格分隔。`1≤n≤2000`，`1≤m≤10^12`；允许用队列并将每轮旋转次数化为 `(m-1) % 当前人数`，最坏时间 `O(n²)`，空间 `O(n)`。
...
```powershell
pnpm lab:doctor -- labs/chapter-02/exercise/E-02-18-josephus-order
pnpm lab:run -- labs/chapter-02/exercise/E-02-18-josephus-order
pnpm lab:score -- labs/chapter-02/exercise/E-02-18-josephus-order
```
