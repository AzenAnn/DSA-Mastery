...
int main() {
    ...
    int key;
    while (std::cin >> op) {
        if (op == "insert") { std::cin >> key; bst.insert(key); }
        else if (op == "find") { std::cin >> key; std::cout << (bst.find(key) ? 1 : 0) << '\n'; }
        else if (op == "remove") { std::cin >> key; std::cout << (bst.remove(key) ? 1 : 0) << '\n'; }
        else if (op == "inorder") bst.inorder();
        else if (op == "height") std::cout << bst.height() << '\n';
    }
    return 0;
}
