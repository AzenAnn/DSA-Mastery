...
# Lab 07-E-05：哥尼斯堡七桥问题

> 题目来源：哥尼斯堡七桥问题的无向多重图推广。
...
给定每座桥的两端，输出一次走完所有桥且不重复的路线，或判定不可行。允许同一对地点之间有多座桥以及自环。为使课程判题唯一，使用如下 Hierholzer 规则：有两个奇度顶点时从较小者出发；否则从最小非零度顶点出发；无边时起点为 0。每次从当前点选择 `(邻点编号, 输入边编号)` 最小的尚未使用边入栈；没有可用边时弹出顶点并追加到逆序答案，最后整体反转。
...
```powershell
pnpm lab doctor labs/chapter-07/exercise/E-07-05-seven-bridges
pnpm lab run labs/chapter-07/exercise/E-07-05-seven-bridges --case 001-sample
pnpm lab score labs/chapter-07/exercise/E-07-05-seven-bridges
```

## 解题思路

先统计奇度顶点并检查非孤立部分连通，再按规则选择起点。使用带独立编号的边运行 Hierholzer：沿未使用边深入，回溯时把顶点加入路径，最后反转路径。
...
2. Hierholzer 回溯时为什么要把顶点加入路径，最后再整体反转？如果直接在第一次到达顶点时加入，会漏掉什么情况？

::: details 参考思路
新环可能从已经访问过的顶点接出，必须先走完当前顶点的所有边，再在回溯阶段拼接子环。回溯得到的是逆序路线，最后反转才能得到正向路径。
:::
