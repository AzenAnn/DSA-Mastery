...
完成前建议先阅读[第 7.3 节 最小生成树](../../../../content/chapter-07-graph-traversal/03-minimum-spanning-tree.md)。你需要准备支持 C++17 的编译器；可先运行 `make doctor` 检查环境。
...
```powershell
pnpm lab:doctor -- labs/chapter-07/exercise/E-07-12-minimum-spanning-tree
pnpm lab:run -- labs/chapter-07/exercise/E-07-12-minimum-spanning-tree
pnpm lab:score -- labs/chapter-07/exercise/E-07-12-minimum-spanning-tree
```
...
::: details 查看参考答案
按边权升序处理时，连接两个不同分量的边是当前割上的最小候选边，切分定理保证加入它不会错过某棵 MST；两端已连通的边会成环，可由环性质安全跳过。选满 `n-1` 条边即得 MST。
:::
2. 存在多重边时，为什么可以只保留最小边权而不影响答案？
::: details 查看参考答案
同端点的较重平行边可替换为较轻边而不破坏连通性，权值不会增加。因此保留最小平行边足以求总权值；保留全部边也可由 Kruskal 自然筛选。
:::
3. 稠密图和稀疏图分别更适合 Prim 还是 Kruskal？依据是什么？
::: details 查看参考答案
本 Lab 的 Kruskal 为 `O(m log m)`；邻接矩阵朴素 Prim 为 `O(n²)`，稠密图通常更合适；稀疏图中 Kruskal 或堆 Prim 的 `O((n+m) log n)` 更有优势。
:::
4. 本题保证图连通；若扩展到不连通图，最小生成树应如何定义，又该如何报告“不存在生成树”？
::: details 查看参考答案
不连通图不存在覆盖全部顶点的生成树；若 Kruskal 结束时选边少于 `n-1`，应按接口输出“不存在”，或明确输出最小生成森林，不能把森林称为 MST。
:::
