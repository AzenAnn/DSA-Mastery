...
无向图存在欧拉回路当且仅当所有非孤立顶点属于同一连通分量且每个顶点度数为偶数。统计度数时自环贡献两次，再从任意非孤立顶点 DFS 检查连通性。
...
2. 为什么“所有顶点度数为偶数”仍不足以保证欧拉回路？请构造一个不连通反例，并说明程序如何排除它。

::: details 参考思路
两个互不连通的环中每个顶点都是偶度，但无法用一条路线走完两部分。程序应从任意非孤立顶点 DFS，并检查所有非零度顶点是否都被访问。
:::
