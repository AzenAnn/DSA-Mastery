...
边权互异只是 MST 唯一的**充分条件**，不是必要条件。一个更精确的检验方法是：先取一棵 MST $T$，再逐条考察非树边 $e$。把 $e$ 加入 $T$ 会形成唯一环；若环上存在与 $e$ 等权的树边，就能交换二者并得到另一棵等权生成树，MST 不唯一。若每条非树边都严格重于它所形成环中的所有树边，则 $T$ 是唯一 MST。边权全部互异时，这些比较不会出现等号，因此 MST 唯一。
:::
...
::: details 参考思路
MST 的正确性来自切分定理，它比较跨越同一切分的边权相对大小，与边权是否为负无关；负权边仍可按正常顺序参与选择。Dijkstra 则依赖边权非负来保证“当前 `dist` 最小的未确定顶点可以永久确定”。若存在负权边，真实最短路径可能先经过当前估计更大的顶点，再通过负权边降低总距离，因此该保证不再成立。
