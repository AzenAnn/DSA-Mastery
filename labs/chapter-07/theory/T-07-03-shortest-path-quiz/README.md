...
- **边界与反例**：负权为何使 Dijkstra 失效、Floyd 如何处理负权并检测负环、最短路径树为何可能不唯一；
...
四个算法都使用松弛操作，但调度顺序不同。试着抽象出一个框架：给定一种“决定下一条松弛哪条边，或下一步处理哪个顶点”的**调度策略**，是否就唯一确定了一个最短路径算法？除了调度，还必须规定哪些状态、更新和终止规则？例如，标准 Dijkstra 的 `done` 版本与不使用 `done`、允许堆中出现重复记录的懒惰版本，为什么都能在非负权图上得到正确结果？
...
概括来说：**负权不会改变“距离估计来自真实路径且只减不增”这两点，但会破坏 Dijkstra 把当前最小估计永久确定下来的依据。**
:::
...
- Floyd 的状态转移 `d(i,j) = min(d(i,j), d(i,k)+d(k,j))` 比较“不经过 k”与“经过 k”两类候选路径，不要求边权非负。它的正确性来自“k 在最外层、按阶段扩展允许的中转点”这一 DP 结构，而不是“当前最小者已确定”的贪心断言。因此，只要相关点对不受负环影响，负边带来的更短路径仍会被正常计算。

但要注意：**Floyd 也怕负环**，只是怕的方式不同——负环让 d(i,i) 持续下降、最短路无定义，Floyd 用对角线 d(i,i)<0 来“报告”而非“解决”。所以更精确地说：

> Dijkstra 依赖非负权来保证贪心确定步骤正确；Floyd 依赖按中转点分阶段的动态规划，因此允许负边。负环则会使受其影响的点对不存在有限最短距离。
:::
