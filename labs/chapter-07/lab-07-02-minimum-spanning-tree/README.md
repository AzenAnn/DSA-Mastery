---
title: "Lab 07-02：最小生成树"
description: "实现 Kruskal 或 Prim 算法求无向图的最小生成树，输出边权之和，并正确处理多重边与自环。"
order: 2
chapter: 7
chapterTitle: "图的遍历与应用"
updated: "2026-08-26"
contributors: ["Fishman"]
status: "draft"
lab: true
difficulty: "基础"
duration: "120～150 分钟"
---

# Lab 07-02：最小生成树

## 学习目标

- [ ] 能说明最小生成树的定义，以及切分定理与环性质；
- [ ] 能用并查集实现 Kruskal，或用优先队列/邻接矩阵实现 Prim；
- [ ] 能正确处理多重边、自环与边权范围；
- [ ] 能说明最小生成树以图连通为前提，并分析算法复杂度。

## 前置知识

完成前建议先阅读[第 7.2 节 最小生成树](../../../content/chapter-07-graph-traversal/02-minimum-spanning-tree.md)。你需要准备支持 C++17 的编译器；可先运行 `make doctor` 检查环境。

## 输入格式

第一行包含两个整数 `n m`，分别表示顶点数和无向边数。顶点编号为 `0` 到 `n-1`。

接下来 `m` 行，每行包含三个整数 `u v w`，表示一条连接 `u` 与 `v` 的无向边，边权为 `w`。输入保证：

- `1 <= n <= 100000`；
- `n - 1 <= m <= 200000`；
- `0 <= u, v < n`（允许自环，即 `u == v`）；
- `1 <= w <= 100000`；
- 图是连通的；可能存在多重边或自环。

## 输出格式

输出一个整数：最小生成树的边权之和。边权之和可能超过 32 位整数，请使用 64 位整数类型。

### 样例输入

```input
4 5
0 1 10
0 2 6
0 3 5
1 3 15
2 3 4
```

### 样例输出

```output
19
```

## 任务

1. 读入全部边，按边权从小到大排序（Kruskal），或用堆维护最小割边（Prim）；
2. 用并查集维护顶点所属连通分量，避免加入使当前森林成环的边；
3. 依次把不构成环的最小边加入生成树，累计边权；
4. 当选中 `n - 1` 条边时停止，输出累计的边权和。

## 正常、边界与错误情况

| 情况 | 预期行为 |
| --- | --- |
| 一般连通图 | 恰好选中 `n - 1` 条边，输出最小边权和 |
| 含环图 | 环上的最大边被跳过，不构成环 |
| 多重边 | 只保留同一对顶点之间更小的边权 |
| 自环 | 自环不改变连通分量，被并查集直接跳过 |
| 单顶点图 | 没有边可选，输出 `0` |
| 线形/星形图 | 生成树即原图本身或最轻的星形 |
| 稠密图 | Kruskal 排序后贪心选择，答案与 Prim 一致 |

## 运行与评分

在本 Lab 目录执行：

```powershell
make doctor
make run
make run CASE=001-sample
make score
```

未安装 Make 时，在仓库根目录执行：

```powershell
pnpm lab:doctor -- labs/chapter-07/lab-07-02-minimum-spanning-tree
pnpm lab:run -- labs/chapter-07/lab-07-02-minimum-spanning-tree
pnpm lab:score -- labs/chapter-07/lab-07-02-minimum-spanning-tree
```

## 完成清单

- [ ] 所有测试用例全部通过；
- [ ] 并查集实现了路径压缩（或按秩合并），避免退化成链；
- [ ] 边权用 64 位整数累加，避免溢出；
- [ ] 多重边会被正确比较，只选更小者；
- [ ] README 中的命令已从干净检出验证。

## 复杂度分析

Kruskal 排序边需要 `O(m log m)`，并查集操作近似 `O(α(n))`，总复杂度 `O(m log m)`、空间 `O(n + m)`。Prim 在邻接矩阵下为 `O(n^2)`，适合稠密图；在堆优化邻接表下为 `O((n + m) log n)`，适合稀疏图。

## 思考与复盘

### 1. 为什么 Kruskal 只考虑“不构成环”就能得到最小生成树？它对应哪条定理？

::: details 参考答案
Kruskal 按边权升序处理边；若加入当前边会成环，就存在一条更晚或等权的环上替代边，可以安全跳过当前边（环性质的交换论证）。不断加入不会成环的最小边，直到得到 `n-1` 条边，即得到最小生成树。
:::

### 2. 存在多重边时，为什么可以只保留最小边权而不影响答案？

::: details 参考答案
同一对端点的较大平行边与较小边具有相同的连通作用。若某棵生成树使用较大边，把它替换为较小边仍连通且不成环，权值不增，因此较大边不可能带来更优答案。
:::

### 3. 稠密图和稀疏图分别更适合 Prim 还是 Kruskal？依据是什么？

::: details 参考答案
邻接矩阵的朴素 Prim 为 `O(n^2)`，适合 `m` 接近 `n^2` 的稠密图；Kruskal 需要排序边，为 `O(m log m)`，稀疏图通常更省存储和排序成本。堆优化 Prim 在稀疏图也常用，最终应结合实现和常数选择。
:::

### 4. 本题保证图连通；若扩展到不连通图，最小生成树应如何定义，又该如何报告“不存在生成树”？

::: details 参考答案
不连通图不存在覆盖全部顶点的生成树；Kruskal 会得到最小生成森林。程序应明确区分两种情况，例如输出 `DISCONNECTED` 或按题面约定报告，而不能把森林权值冒充生成树答案。
:::
