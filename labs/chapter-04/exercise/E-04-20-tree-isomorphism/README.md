...
```powershell
pnpm lab:doctor -- labs/chapter-04/exercise/E-04-20-tree-isomorphism
pnpm lab:run -- labs/chapter-04/exercise/E-04-20-tree-isomorphism
pnpm lab:run -- labs/chapter-04/exercise/E-04-20-tree-isomorphism --case 001-sample
pnpm lab:score -- labs/chapter-04/exercise/E-04-20-tree-isomorphism
```
...
vector<int> findCentroids(int n, const vector<vector<int>>& adj) {
    vector<int> sz(n + 1, 0);
    function<void(int, int)> dfs = [&](int%20u,%20int%20p) {
        sz[u] = 1;
        for (int v : adj[u]) {
            if (v == p) continue;
            dfs(v, u);
