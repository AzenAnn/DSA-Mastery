...
::: tip 关于根节点
树以节点 1 为根。由于必须恰好选 $k \ge 1$ 个节点且选任何节点都必须先选其父节点，根节点 1 作为整棵树的公共祖先必然在团队中。即便根节点能力值为负数，为了选入能力值更高的子节点，根节点也必须被选中。
:::
...
根节点 1 的能力值为 $-5$。根据依赖规则，若选任何子节点都必须先选择其直属上级。  
因此组建 $k=2$ 人的团队时必须包含根节点 1：
- 选节点 $\{1, 2\}$：总和 $= -5 + 3 = -2$。
- 选节点 $\{1, 3\}$：总和 $= -5 + 4 = -1$。  
...
```powershell
pnpm lab:doctor -- labs/chapter-04/exercise/E-04-28-research-team-formation
pnpm lab:run -- labs/chapter-04/exercise/E-04-28-research-team-formation
pnpm lab:run -- labs/chapter-04/exercise/E-04-28-research-team-formation --case 001-sample
pnpm lab:score -- labs/chapter-04/exercise/E-04-28-research-team-formation
```
...
- $k = 1$ 时只能选根节点；
- 无论根节点价值正负，只要 $k \ge 1$，根都必须选（否则无法选任何节点）。
