...
```powershell
pnpm lab:doctor -- labs/chapter-04/lab-04-27-longest-zigzag-path
pnpm lab:run -- labs/chapter-04/lab-04-27-longest-zigzag-path
pnpm lab:run -- labs/chapter-04/lab-04-27-longest-zigzag-path --case 001-sample
pnpm lab:score -- labs/chapter-04/lab-04-27-longest-zigzag-path
```
...
1. 为什么需要同时维护「第一步向左」和「第一步向右」两个状态？只用一个状态能否解决问题？
2. 如果路径允许向上回溯（如从叶子到叶子），之字形路径的定义需要如何调整？
3. 能否用 Morris 遍历等 $O(1)$ 额外空间的方法解决本题？
