...
- [ ] 单节点、链、星形等边界自测通过
- [ ] 程序满足 $O(n)$ 时间复杂度
- [ ] 能理解换根 DP 的递推公式 $dp[v] = dp[u] + (n - 2 \times size[v]) \times w$

## 思考题

1. 为什么先任选一个根（如节点 1）做一次 DFS，记录子树大小和子树内距离和？这两个数组在换根时各自起什么作用？
2. 如果边权不一定是正整数，换根 DP 的公式是否仍然成立？
3. 若只要求找出总成本最小的节点（而不需要输出所有节点的成本），算法是否可以简化？
