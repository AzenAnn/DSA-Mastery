---
title: "Lab 13-E-08：K 次取反后最大化的数组和"
description: "用排序和剩余次数奇偶性最大化数组和。"
order: 8
chapter: 13
labId: "13E08"
chapterTitle: "贪心算法"
...
必须恰好执行 `k` 次：每次选择一个元素取相反数。求可能得到的最大数组和。
...
排序后先翻转负数；若次数仍有剩余，只有剩余次数为奇数时才翻转绝对值最小的元素。

## 复杂度分析

时间 `O(n log n)`，额外空间取决于排序实现。
...
本题对应 LeetCode：<https://leetcode.com/problems/maximize-sum-of-array-after-k-negations/description/>。本 Lab 将函数式参数改写为标准输入输出格式。
