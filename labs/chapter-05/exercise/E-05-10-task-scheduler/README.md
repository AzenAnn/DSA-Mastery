...
任务频次：$1$ 出现 $3$ 次，$2$ 出现 $2$ 次，$3$ 出现 $1$ 次。冷却时间 $k=2$，即两个相同任务的位置之差至少为 $3$。
...
- 时间 $2$ 执行 $2$
- 时间 $3$ 执行 $3$
- 时间 $4$ 执行 $1$（距上次 $1$ 正好间隔 $2$ 个时间单位，满足）
- 时间 $5$ 执行 $2$（距上次 $2$ 间隔 $2$，满足）
- 时间 $6$ 无可用任务（剩下的 $1$ 还在冷却中），空闲
- 时间 $7$ 执行 $1$（距上次 $1$ 间隔 $2$，满足）

总共需要 $7$ 个时间单位。注意不能压缩到 $6$：三个 $1$ 两两之间至少要隔 $2$ 个时间单位，仅容纳它们就需要 $(3-1) \times 3 + 1 = 7$ 个时间单位。
