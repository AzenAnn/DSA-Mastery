...
`xy3` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/xy3/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/xy3/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee xy3` to
...
python3 ./.trellis/scripts/task.py archive 00-join-xy3
```
