...
- `pnpm test` 必须包含 Lab Schema/路径/比较器/进程限制单测和作者指南示例检查；内容 validator 遍历所有三层目录中的 Lab，README-only Lab 同样必须满足分类、路径和标题合同。
...
- Lab 路由迁移：静态合同断言 173 个三级地址全部生成、旧平铺地址均不生成；第 1 章分类基线为 6/15/2，Theory 包含 README-only 的 01T06 理论大题训练，Project 包含 01P01 评测器与 01P02 105 栋的热心同学。Pages 浏览器覆盖四层原生折叠、三色三图标、明暗主题与 390px 无溢出。
