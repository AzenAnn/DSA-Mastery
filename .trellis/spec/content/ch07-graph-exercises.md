...
pnpm run test:ch07
pnpm lab:verify -- labs/chapter-07/exercise/E-07-32-min-cost-max-flow
```
...
- T01–T30 是清单顺序，不是新 Lab 身份；order=101..130。既有 07E01–07E12 永久保留，新题为 07E13–07E32，07E02/07E03 补充项 order=191/192。不能为了排列整齐改 ID、目录或复用旧号。
...
- 07E13：DFS 从 0 起并按编号补扫森林，邻接按 (终点,边号)；自环为 BACK，平行边分别分类。
- 07E01 仍教授递归 DFS，但深链在 Windows 默认栈上可能溢出。参考解递归深度 256 后用等价显式栈帧完成子树；保留原有深链测试，不依赖操作系统专属栈配置。
- 07E14/15：连通性仅看非零度点，自环度数+2；无边有零长度回路。路径起点为较小奇度点；07E15 规范化 Hierholzer 后验验证 m+1 点和边多重集。
- 07E17：每步选择当前最小零入度顶点。07E30：固定 DFS 增广顺序并按左端点排序；最大数量由独立网络流检查。任意方案题改成唯一输出时必须在题面明示。
- 07E19：源到汇的完整链计数模 80112002，孤立点计 1。07E21：所有汇事件使用全局工期，禁止每个分量单算最迟时刻。
- 07E25 检测全图负环，不与既有 07E11 的源点可达负环语义混淆。07E27 不可达距离视为无穷，同时单独要求 h(t)=0 和全图每条有向边的一致性。
- 07E28 目标固定 123804765，0 不计入启发式，以字符串读入；课程扩展无解=-1。
- 07E31/32 每条输入边有独立残量反向边；费用取相反数；64 位存容量、流量、路径时刻和费用乘积。费用流保证初始无负费用环，先最大流再最小费用。
...
| 条件 | 必须结果 |
| --- | --- |
| T 映射缺项、稳定 ID 改写或 order 错 | test:ch07 失败 |
...
先运行 test:ch07 与作者独立 oracle，再运行全部 32 道 Program 的 lab:verify；标准 pnpm test 完成内容、类型、lint、发现、构建、产物链接。抽查最终站点从 Labs 入口到新增题的真实导航。--mutations 保留每题被杀死的具体测试点。输出 .out 必须逐字节为 LF，README 样例检查先规范 CRLF/LF。
