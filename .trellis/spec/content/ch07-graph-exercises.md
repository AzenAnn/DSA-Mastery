...
- 2026-09-16 维护者明确授权按既定顺序重编号：T01–T30 对应 07E01–07E30，order=101..130。目录、labId、标题与学习顺序一致；删除旧连通分量计数、有向图环检测。迁移记录见 docs/ch07-exercise-order.md。后续新增仍遵守稳定 ID 默认规则，不自动重排。
- 练习入口直接放在对应章节文章中；不再维护单独的 Ch7 题目清单页面。Lab README 面向学生，只保留题面、算法前置、测试约束、解题思路和复盘内容，删除题集规划、重编映射等维护过程描述。
