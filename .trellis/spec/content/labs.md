...
2026-09-11 Ch4 为维护者明确要求的重编号例外：保留 20 道旧题，加入 14 道新题，清单 31 项后追加 3 道未列旧题。迁移记录见 [Ch4 编号清单](../../../docs/ch04-exercise-order.md)。该例外同时迁移 VS Code 进度、事件与旧目录别名；后续插题仍遵守默认稳定 ID 规则，不得依此自动重排已发布身份。
...
复杂度单选题必须区分上界与紧界。若要唯一选出平方级递推，应问 `Θ(n²)`；只问 `O(n²)` 会同时接受线性、线性对数等更小量级。代码题若沿用 O 量级选项，应明确“选择最紧的量级”。精确计数需检查零输入、完全平方数、2 的幂及相邻值；例如 `i=n*n; while(i!=1) i/=2` 在正整数且不溢出时，改成 `i!=0` 只多一次除法，不改变对数阶。维护解析时同步教材入口与反例反馈，并实际运行代表边界。
...
2026-09-16 Ch7 按维护者明确要求重编号为 07E01–07E30，并删除旧 07E02/03；一次性例外见 [Ch7 编号迁移](../../../docs/ch07-exercise-order.md)。必须同时迁移进度及事件，隔离删除题历史；不能把此例外推广为自动重编号。
