...
- “本章 Labs”使用独立边框、轻背景和圆角；Theory/Exercise/Project 分别复用 accent/success/signal 语义色与 Lucide BookOpen/FlaskConical/Blocks 图标。三类必须同时显示中英文名称；任一类别无内容时仍以可折叠分组展示对应的“暂无理论型/实验型/工程型 Lab”。Ch.5 在全部为空时也必须保留完整接口。
