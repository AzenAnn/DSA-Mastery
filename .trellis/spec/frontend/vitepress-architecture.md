...
content/chapter-preface/02-windows-student-setup.md
                                -> learn/chapter-preface/02-windows-student-setup/index.md
content/chapter-preface/03-lab-cli-command-guide.md
                                -> learn/chapter-preface/03-lab-cli-command-guide/index.md
content/chapter-preface/04-graphviz-authoring-guide.md
                                -> learn/chapter-preface/04-graphviz-authoring-guide/index.md
...
- 未拆分的课程章节使用 `autoLabChapter` 从 ContentIndex 自动收录对应物理 chapter 的全部 Lab；不得用 `labSources` 维护一个会漏掉新增 Lab 的手写子集。所有声明 `autoLabChapter` 的章节把同一自动收录集合按 `labCategory` 展示为“本章 Labs”的 Theory/Exercise/Project；即使集合为空也渲染三类固定空状态。其他章节继续显示“相关 Labs”。只有同一物理 chapter 被编排为多个课程章节时，才使用显式 `labSources` 分流，并说明边界。
...
- 前言是 ContentIndex 中唯一的非数字 `ChapterId`：固定为 `"preface"`，通过显式 `label: "前言"` 与 `chapterRank = -1` 排在 Ch.0 之前。它使用真实章节 outline 依次列出理论环境展示、Lab 更新、Windows 环境、Lab 命令接口和 Graphviz 图示五篇完整指南；长指南通过 include 复用 `docs` 单一事实来源，不改变数字章节编号。
...
| 前言通过负数/大数模拟、遗漏五篇指南中的任一篇、复制长指南正文或改变数字章节 | 内容/架构检查失败 |
...
| 声明 `autoLabChapter` 的章节在零 Lab 时不显示三类空槽位，或产生占位 Lab URL | artifact/Pages 失败；始终渲染 `chapterLabGroup([], icons)`，且不得创建虚假 README |
| 自动收录章节同时维护 `labSources` 子集 | 架构检查失败；删除重复来源 |

## 5. Good / Base / Bad Cases

- Good：config 和 loader 从同一收集器取得已排序条目；默认主题负责搜索与文档外壳，Vue 只消费序列化结果。
- Good：前言定义使用 `/learn/outline/chapter-preface/`，并在 `lessonSources` 中显式列出 `00`～`04` 五篇站内指南。
...
- 分类章节的 README-only fixture 必须显式声明 `labCategory`，并证明它进入对应分类；产物与浏览器测试还要断言第 1 章的实际数量与折叠行为，以及 Ch.5 在零 Lab 时仍输出三个无链接空分类。
- `pnpm run build && pnpm run check:site` 核对 ContentIndex 返回的全部教材/Lab、首页、Labs 索引、404、链接与恰好一个 base。
- 前言产物检查使用 POSIX 化后的相对路径筛选；断言恰好五篇文章、章节 outline、站内搜索和侧栏入口全部存在。Windows 的 `path.join()` 会产生反斜杠，不能直接与 `learn/chapter-preface/` 比较。
...
```ts
// Wrong：零 Lab 时整个接口消失。
chapter.labs.length ? [chapterLabGroup(chapter.labs, icons)] : []

// Correct：autoLabChapter 是启用分类接口的合同，与当前题目数量无关。
