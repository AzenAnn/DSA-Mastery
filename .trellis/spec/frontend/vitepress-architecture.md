...
- 前言是 ContentIndex 中唯一的非数字 `ChapterId`：固定为 `"preface"`，通过显式 `label: "前言"` 与 `chapterRank = -1` 排在 Ch.0 之前。它使用真实章节 outline 依次列出理论环境展示、Lab 更新、Windows 环境和 Lab 命令接口四篇完整指南；后三篇通过 include 复用 `docs` 单一事实来源，不改变数字章节编号。
...
| 前言通过负数/大数模拟、遗漏四篇指南中的任一篇、复制长指南正文或改变数字章节 | 内容/架构检查失败 |
...
- Good：前言定义使用 `/learn/outline/chapter-preface/`，并在 `lessonSources` 中显式列出 `00`～`03` 四篇站内指南。
...
- 前言产物检查使用 POSIX 化后的相对路径筛选；断言恰好四篇文章、章节 outline、站内搜索和侧栏入口全部存在。Windows 的 `path.join()` 会产生反斜杠，不能直接与 `learn/chapter-preface/` 比较。
