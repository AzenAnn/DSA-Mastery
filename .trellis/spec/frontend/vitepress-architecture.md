...
- 前言是 ContentIndex 中唯一的非数字 `ChapterId`：固定为 `"preface"`，通过显式 `label: "前言"` 与 `chapterRank = -1` 排在 Ch.0 之前。它使用真实章节 outline 列出理论环境展示、Lab 更新、Windows 环境、Lab 命令接口、Graphviz 图示和 macOS 环境六篇完整指南；长指南通过 include 复用 `docs` 单一事实来源，不改变数字章节编号。
